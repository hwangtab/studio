...
⚠️ **날짜 규약.** 화면은 날짜만 고르고 시각은 코드가 붙인다. **시작일은 `T00:00:00+09:00`, 종료일은 `T23:59:59+09:00`이다** — 개설자가 고른 마지막 날은 하루 종일 모금이 열려 있어야 한다(기존 `content/funding/*.md`도 같은 규약이다).

**오프셋을 반드시 명시한다.** `new Date('2026-10-05T00:00:00')`은 실행 환경의 로컬 자정이라 서버(UTC)와 브라우저(KST)가 갈리고, 되읽을 때 `iso.slice(0, 10)`으로 자르면 **저장할 때마다 날짜가 하루씩 뒤로 밀린다.** 서버가 `YYYY-MM-DD`(Asia/Seoul)로 내려 주고 폼이 오프셋을 붙여 돌려보내는 한 방향으로만 변환한다.
