...
- 작성일: 2026-09-25
- 상태: **운영자 결정 확정(2026-09-25, §9) · 1·2·3단계 구현.** 3단계는 DB 없이 연결만 했다 — 관리자 화면의 "설계 대행 프로젝트" 표시는 `internal_note`로 적는다(새 컬럼·마이그레이션 없음).
...
| **3** 플랫폼 연결 | `/ko/funding` 목록 아래 세 갈래(직접 개설·설계 대행·발매 프로젝트), `/ko/funding/apply`에 "혼자 쓰기 막막하다면" 안내, 사례 카드에 남산타워 발매기 스토리 연결. 설계 대행 프로젝트 표시는 관리자 `internal_note` | 소 | 완료(2026-09-26) |
