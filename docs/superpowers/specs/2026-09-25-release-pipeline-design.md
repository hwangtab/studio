...
6. **티어 표** — 가격 문구는 그대로(로케일 티어 가드가 숫자를 파싱한다). 표 아래 주석 한 줄 추가:
   "세션 연주비·아트워크는 실비 별도 · 펀딩 경로면 제작비는 모금액에서 집행".
   세션 포함 여부가 페이지마다 다르던 것("세션 1~2인 포함" vs 정본 없음)은 §9-7 결정으로 "실비 별도, 섭외 수수료 없음"으로 맞췄다.
...
| 7 | 세션 연주비(2026-09-26) | **번들·티어에 포함하지 않는다. 연주자 실비만 받는다(섭외 수수료 없음)** | 싱글·EP·정규 페이지 7개 로케일의 "세션 N인 포함·추가 편성 별도 견적" 문구를 "연주비 실비 별도"로 통일. 발매 허브·llms·가격 주석도 같은 문장 |
| 8 | 홍보 범위(2026-09-26) | **국내·해외 모두 번들에 포함한다. 별도 옵션으로 떼지 않는다** | 발매 LP의 범위 목록·프로세스 마지막 단계를 7개 로케일에서 "국내외 매체·라디오·플레이리스트"로 맞췄다. 번들·티어 구성 문구는 이미 그렇게 약속하고 있었다 |
| 9 | LP 실험 잠금(2026-09-26) | **11월까지 기다리지 않고 지금 바꾼다** | `/ko/release-project` 첫 화면 제목 "곡은 있는데 돈이 막막하다면, 제작비부터 만들어 드립니다", SEO 타이틀, 첫 화면 버튼을 "발매 자금 상담(무료 30분)" 하나로. 실험 기록은 ⏹ 중단, 9/27부터 잠금 없이 관측 |
