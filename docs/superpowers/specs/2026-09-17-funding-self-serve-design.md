...
4. ✅ 관리자 프로젝트 탭·판정·알림·revalidate (3차 계획 문서 `2026-09-18-funding-admin-review.md`).
   관리자 목록·상세·판정(승인·보완 요청·반려), 승인 시 slug 확정과 `lockedAt` 기록, 판정 알림
   메일과 운영자 폴백 메일, 온디맨드 `res.revalidate()`(목록·상세 양쪽), 대시보드 대기열,
   개설자 약관 본문과 `FUNDING_CREATOR_TERMS_VERSION` 실제 값, 후원자 약관 개정과
   `FUNDING_TERMS_VERSION` 인상, 공개 상세의 개설자 표시, 프로젝트 보관(archive)까지.
   **여기까지 코드는 끝났고, 첫 DB 프로젝트 공개는 운영자의 변호사 확인 뒤다.**
5. 공개 뒤 편집 규칙·`lockedAt` 가드·개설자 통계 — **4차로 넘어간다.**
6. 정산 계산·관리자 정산 탭·개설자 약관 수수료 조항·llms/사이트맵 반영 — **4차로 넘어간다.**
   개설자 약관 제6조가 지금 "별도 정산 계약으로 정한다"로 비어 있는 것이 그 자리이고,
   배송지를 개설자에게 넘기는 처리위탁도 경로를 만드는 4차에서 처리방침에 넣는다.

4단계는 공개 화면을 한 줄 바꾼다 — DB 프로젝트 상세에 "개설자 {이름} · 판매자 스튜디오 놀"
표시가 붙는다(후원자 약관 제4조가 약속한 것). 마크다운 프로젝트는 `creator`가 null이라
화면이 그대로다(2026-09-18 확인: 빌드 산출물 `keep-singing-for-palestine.html`에 "개설자"
0건, `__NEXT_DATA__`는 `"creator":null`).

각 단계가 독립 배포 가능하고, 3까지는 공개 화면에 아무 변화가 없다(2026-09-17 확인:
`/ko/funding`·`/ko/funding/[slug]` 모두 ISR revalidate 1m 그대로, 빌드 산출물에 공개 경로
쪽 신규 항목 없음 — 새로 생긴 라우트는 전부 개설자 흐름 `/[locale]/funding/creator*`와
`/api/funding/creator/*`, `/api/funding/media/*`).
