...
4. 관리자 프로젝트 탭·판정·알림·revalidate. 여기서 첫 DB 프로젝트가 공개된다. **다음은 3차
   계획 문서로 넘어간다** — 관리자 프로젝트 목록·상세·판정(승인·보완 요청·반려), 승인 시
   slug 확정과 `lockedAt` 기록, 판정 알림 메일, 온디맨드 `res.revalidate()`(목록·상세
   양쪽), 개설자 약관 본문과 `FUNDING_CREATOR_TERMS_VERSION` 실제 값, 후원자 약관 개정과
   `FUNDING_TERMS_VERSION` 인상(첫 DB 프로젝트 공개 전 — 변호사 확인 항목).
