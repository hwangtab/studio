...
12개월 목표는 월 매출 +300~400만원(발매 파이프라인이 분기 2건이면 +450만원 안팎)이다. 초안의 600~900만원은 운영자 1인의 과금 가능 시간(월 100~110시간)을 무시한 약 2배 과대 추정이었다.
...
| 펀딩 설계 | 스토리·리워드·페이지 | 40만 + 모금액 10%(810만이면 약 81만) | 40h |
| 제작·홍보 | EP 번들: 기획·녹음·믹싱·마스터링·국내 홍보 | 180만(세션·아트워크 실비 별도) | 85h |
| 유통 | 협력 유통사 연결 | — | 1~2h |
| 합계 | 3~6개월 | 약 300만 | 약 125h(월 25~30h) |

**시간당으로는 약 2.4만원이라 녹음 세션보다 낮다. 그래도 핵심 축인 이유는 세 가지다.**
...
- **세션·아트워크는 "실비 + 섭외 수수료 10~15%"로 명시한다.** 스튜디오가 떠안으면 번들이 적자가 된다.
- **펀딩 선불 40만원은 성공 수수료에서 차감하고, 총비용 계산기를 공개한다.** 모금 353만원 수준의 소형 펀딩은 스튜디오 몫과 플랫폼 수수료가 모금액의 약 32%라, 고객이 계산하기 전에 우리가 보여야 한다. 목표 500만원 미만은 셀프 가이드로 안내하는 것도 방법이다.
- **홍보는 번들에 국내 1라운드를 넣고, 해외 확장은 정가 옵션으로 둔다.**
- **유통은 협력사 연결이라 운영자 시간이 거의 들지 않는다.** 이 사실을 페이지에 명시해 "유통까지 책임진다"를 보여준다.
- **신뢰 숫자를 맞춘다.** 배지 "누적 약 3억원"과 공개 표의 검증 합계(약 1.35억원)를 정합하고, 미달 건을 포함한 성공률(18/19)을 같이 싣는다.
- **와디즈 개인 메이커 수수료 지원이 2026년 말에 끝난다.** 그 전 개시를 세일즈 포인트로 쓴다.
...
| 펀딩 설계 (모금 500만~1,000만) | 90~140만 | 40h | 2.3~3.5만 | 연 4~6건 |
| 싱글 번들 | 50만 | 35h | 1.4만 (세션비 부담 시 0.2만) | 월 1건 |
| 음원 홍보 도입가 | 30만 | 43h | 0.7만 | 월 3팀 |

**세션비 표기가 엇갈린다.** 발매 페이지 문구는 "세션 1~2인 포함"이라고 하는데 가격 정본에는 세션 항목이 없다. 서울 세션은 곡당 인당 20~30만원이라 스튜디오가 떠안으면 번들은 적자에 가깝다.
...
1. **미머지 브랜치를 머지하고 연습실 빈방 1개를 채운다 (2~3h).** 펀딩 설계 페이지와 시간제 안내가 즉시 열린다. 빈방은 대기자에게 직접 연락해 채운다. 이것만으로 월 +36만원이다.
...
5. **네이버 파워링크를 축가·성우에만 건다 (셋업 4h, 월 21만원).** 두 상품은 이미 토스 예약이 붙어 있어 클릭이 바로 결제로 이어진다. 집행 전 키워드도구로 CPC를 확인한다.
...
| 채널 | 실행 | 월 비용 | 착지 | 판정 |
| --- | --- | --- | --- | --- |
| 네이버 파워링크 | 축가 녹음·성우 녹음실·나레이션 녹음실 같은 롱테일. 합격하면 믹싱 의뢰·음원 발매 대행 추가 | 21만 | 축가·성우 LP(예약 결제 있음) | 4주, 결제 기준 건당 비용 |
...
| 발매 번들 | 세션·아트워크는 "실비 + 섭외 수수료 10~15%"로 명시. 싱글 65~70만 재가격 검토 | 표기는 즉시, 가격은 성사율 본 뒤 |
...
| 발매 파이프라인(펀딩·지원사업 연계) | 분기 1건 × 약 300만 | 약 100만 |
| 홍보 정가 | 2027년 월 1팀 × 120만 | 약 60만 |
| 파워링크·플레이스 경유 축가·성우 | 월 2~3건 | 약 60~90만 |
| 합계 | | 약 350~380만 |

발매 파이프라인이 분기 2건이면 월 약 100만원이 더해져 450만원 안팎이다.
