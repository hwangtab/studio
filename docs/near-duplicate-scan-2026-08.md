# 스토리 근접중복 전수 스캔 — 2026-08-28

> **7월 스캔(near-duplicate-scan-2026-07.md) 대비 등재 페어 81건 → 40건.** 줄어든 원인은
> 리다이렉트가 아니라 보일러플레이트 제거다. 2026-08-25~28 라운드에서 워드카운트 패딩이
> 남긴 동일 섹션을 290편에서 걷어내고(12.1만자), 지역 공용 블록 56편을 도시별로 재작성했다.
> 스캐너는 AUTO-EXPAND 블록만 제외하고 본문의 복붙 섹션은 유사도에 포함하므로, 그 정리가
> 그대로 페어 수 감소로 나타난다. 방법론·임계는 7월과 동일해 두 수치는 직접 비교 가능하다.
>
...
> 전체 페어 데이터(지역·noindex 포함 40건): [near-duplicate-scan-2026-08.csv](near-duplicate-scan-2026-08.csv)
...
- 비교 페어: 594,595건 → 크기 필터 통과 548,902건 → 스케치 통과 615건 → **J ≥ 0.45 등재 40건**
- GSC 매칭: 대상 1091편 중 1075편이 CSV에 존재
- 소요 시간: **1.1s** (전처리 0.7s + 페어 스캔 0.4s)
...
## 실행 대상 — 양측 모두 색인 가능한 페어 (34건)

양측 모두 robots noindex가 아니고 thin-content 게이트에도 걸리지 않는, 즉 **둘 다 실제로 색인되는** 페어. 이 중 지역 카테고리 페어 34건은 지역 정책 트랙(§1-6·역세권 pSEO)에서 다룰 사안이라 아래 "지역·noindex" 절에 건수·요약으로 분리했고, **비지역 0건**이 페어 단위 통합(강자 canonical + 약자 308) 검토의 실질 대상이다.
...
*생성: `scripts/scan-near-duplicates.mjs` (2026-08-28, 분석 전용 — 콘텐츠 비수정)*
