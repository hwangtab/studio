...
> Copilot(Bing)** — 은 전부 **플랫폼 장소 DB·리뷰·다른 색인**을 읽는다. 그리고 그 DB에 지금 **틀린 사실이 올라가 있다**
> (카카오맵 전화번호가 kosmart 번호, 구글에 "폐업" 중복 리스팅).
>
...
| **카톡 안의 ChatGPT** (ChatGPT for Kakao) | 누적 가입 1,300만(2026-08 컨콜) [보도] | 장소 질문에 **Kakao Tools가 카카오맵을 호출**해 위치·정보를 준다 [공식, 원문 확인] | [실측] 카카오맵에 "스튜디오 놀 불광점"으로 존재하나 **전화 02-764-3114 = kosmart 번호**(`docs/outreach/kosmart-redirect-2026-09-21.md`). 운영자 관리 흔적 없음(runbook 미체크) |
...
| **구글 AI Overviews·AI Mode·Gemini 앱** | Gemini MAU 947만 [통계]. AI Mode 한국어 2025-09 [공식] | 구글 색인(Googlebot) + **Business Profile·Maps** [공식]. Gemini 앱 grounding은 `Google-Extended`로 통제 [공식]. 에이전트 예약·대리 전화는 **미국 한정** [공식] | GBP 인증 완료(8/24). **같은 사업장의 "폐업" 리스팅이 정확한 상호("스튜디오 놀")로 남아 있다**(`data/siteConfig.ts` 주석) — 정리 작업이 점검표에 없다. Gemini는 믹싱 질문에 웹을 안 연다(0/6) |
...
| # | 결함 | 실측 | 어디서 고치나 |
|---|---|---|---|
| D1 | **카카오맵 전화번호가 kosmart 번호(02-764-3114)** | 카카오맵 검색 `confirmid 482217521` "스튜디오 놀 불광점 · 통일로71길 2-1 · 음악 · 02-764-3114" | 카카오비즈니스 파트너센터(장소 관리) 또는 카카오맵 앱 정보 수정 제안 |
| D2 | **구글 "폐업" 중복 리스팅 미정리** | `siteConfig.ts` 주석: CID 5451462716481993990 "스튜디오 놀"(폐업·소유권 없음). `docs/console-checklist-2026-09.md`·runbook에 처리 항목 0건 | Google 지도 → 해당 장소 "수정 제안 → 중복" / GBP 지원 문의 |
...
| # | 작업 | 방법 | 소요 |
|---|---|---|---|
| **P0-1** | **카카오맵 장소 정정(D1)** | 카카오비즈니스 파트너센터에서 장소 `482217521` 관리 권한 신청 → 전화 010-4255-7893, 홈페이지 studionol.co.kr, 영업 24시간, 업종(녹음실·연습실 계열 선택지 확인), 대표 사진. 소유 인증이 막히면 카카오맵 앱 "정보 수정 제안"으로 전화번호부터. 완료 확인은 카카오맵 검색 결과의 `tel` 값 | 30분 + 검수 대기 |
| **P0-2** | **구글 폐업 중복 정리(D2)** | 구글 지도에서 CID 5451462716481993990 → "수정 제안 → 이 장소는 중복임" 선택, 활성 리스팅(CID 17692560696856302422)을 지정. 반영 안 되면 GBP 지원 채팅. 점검표에 항목 추가 | 15분 |
...
| KPI | 기준선 | 어디서 | 주기 |
|---|---|---|---|
| **플랫폼 사실 오류 수**(전화·주소·상호·폐업 표시) | 2건(D1·D2) → 0 | 카카오맵 검색 `tel`, 구글 지도, P1-2 `facts_wrong` 칸 | P0 직후, 이후 월간 |
