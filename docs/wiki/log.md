...
## 2026-07-20 · ingest · 품질 심화 배치6~7(80편, 누적 258편)
- 배치6: mixing 강좌 시리즈 13편 포함 40편. 시리즈 내부 Jaccard 0.025로 각 편 소주제 특화 성공
- 배치7: 피아노·팟캐스트·pitch·playlist 밀집 클러스터 40편
- 서브에이전트가 탈템플릿을 넘어 실질 교정 수행: 박스-본문 불일치(mixing12), 타 글과 중복 박스 주제(mixing10),
  자기모순 수치(pop-production -9~-11 vs -14 LUFS, mv1 -16 vs -14, neo-soul 하이컷), 오타·깨진 리스트/들여쓰기,
  미검증 수치 제거(세금 4,800만원·팟캐스트 -16LUFS·Waves Tune)
...
## 2026-08-14 · lint · 포트폴리오 발매일 전수 대조 (34건)
- 젠트리피케이션 메타 오류를 계기로 items.ts 34개 항목의 releaseDate를 본문(productionNotes.ko)과 전수 대조
- **본문이 정본**이라고 판정. 근거: 운영자 이력과 교차검증 가능한 5건(물고기는·강호중·여린잎·엉아들·젠트리피케이션)에서
  본문이 전부 일치하고 메타만 어긋났다
- **27건 교정**: 큰 차이 11 + 하루 차이 6(본인 확인 "그냥 오차") + releaseDate 필드 누락 9 + peace-and-music 1
  - 최대 오차 605일(the-projectors), -555일(물고기는 물이 없으면 죽어요)
  - peace-and-music〈이름을 모르는 먼 곳의 그대에게〉 앨범 발매일은 본인 확인으로 **2025-02-07**.
    단 크레딧·레드어워드 연도는 2024로 유지된다(프로젝트 진행 연도 기준, 2026-08-14 본인 지시).
    수록곡 선공개는 2024-11(남수·모레도토요일) — 앨범 발매일과 트랙 공개일이 다른 케이스다
- label 필드도 함께 대조: 젠트리피케이션 외 이상 없음(나머지는 아티스트명=자체 발매 레이블 형태)
- **미해결**: `guitar-choi-dementia-2024`·`jinu-konda-burn-in-hell`·`balkwaehan-cm-song` 3건은
  productionNotes 블록 자체가 없어 상세 페이지 설명이 비어 나간다. 사실을 지어낼 수 없어 보류 —
  발매일·참여자·작업 범위를 확인해 채울 것

## 2026-08-20 · lint · about 페이지 서비스 카드 ↔ 서비스 정본 대조
- 계기: 한국어 윤문 라운드에서 `data/services.ts`의 음반 제작 8단계를 훑다가 정본 미대조 항목 발견
- **entities/services.md 갱신**: §8에 "라이브 공연 기획·운영 지원" 등재 (본인 확인 2026-08-20).
  사이트에는 노출 중인데 정본·pricing.ts 어디에도 없어 다음 점검에서 또 걸릴 항목이었다. 가격 미정
- **data/services.ts 3건 교정** — 정본에 근거가 없어 과대 서술이던 것:
  - 디자인 "아트워크·자켓·프로모션 이미지 제작을 지원합니다" → 외주 디자이너 연결·비용 별도
    (발매 프로젝트 deliverablesExcluded "커버 아트워크(외주 디자이너 연결 가능, 비용 별도)"와 정합)
  - 유통 "온라인/오프라인 음원 및 음반 유통" → 디지털 플랫폼 유통 등록으로 한정.
    오프라인(피지컬) 유통은 정본에 근거 없고, 발매 프로젝트는 CD·LP 제작을 excluded로 명시
  - 홍보 "SNS 활용, 언론 배포, 온오프라인 홍보 지원" → 보도자료 작성·매체 배포로 한정
    (정본 §8은 "기본 홍보 패키지(보도자료 + 미디어 배포)"까지. SNS 광고비는 발매 프로젝트 excluded)
- 미해결 없음. 8단계 중 나머지 5개(기획·펀딩·레코딩·믹싱마스터링·공연기획)는 정본과 일치

## 2026-08-20 · lint · 카피 감사 범위 정정 (앞 항목 되돌림)
- 앞 항목의 "디자인 → 비용 별도 명시"와 별건으로 진행했던 "전문가 연계" 문구 축소를 **되돌린다**(본인 지시).
- 되돌린 이유: 감사 범위가 "사실 오류·자기모순" 밖으로 나가 **마케팅 카피의 설득력까지 깎고 있었다.**
  "최고의 전문가", "최상의 결과물" 같은 최상급은 사실 문제가 아니라 카피 영역이고 판정자는 운영자다.
- 복원: 전문가 연계 카드 "각 분야 최고의 전문가들과 협업하여 최상의 결과물을 보장합니다"(7개 로케일 원문)
- 축소: 디자인 카드에서 "비용은 별도입니다" 삭제 — 외주 연결 사실만 남김
- **이후 기준**: 이 저장소의 카피 감사는 (1) 다른 페이지와 모순되는 서술, (2) 기술적으로 거짓인 주장,
  (3) 문법 오류, (4) 정본에 없는 서비스까지만 손댄다. 최상급·감성 표현·비교 우위 문구는 대상이 아니다.
