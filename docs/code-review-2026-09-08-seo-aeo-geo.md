...
1. ~~**#4 바이라인 방향**~~ → **2026-09-09 결정: 스키마를 화면에 맞춘다.** 스토리는 스튜디오
   명의로 쓰므로 author도 Organization(스튜디오 놀)이다. 운영자 Person은 frontmatter에
   `author: 황경하`를 명시한 48편에서만 나간다. 리뷰어 권고(화면을 스키마에 맞춰 전 스토리에
   실명 노출)는 채택하지 않았다 — 사실이 아닌 쪽으로 맞추는 것이기 때문이다.
   Person entity의 GEO 가치는 사라지지 않는다: `Organization.founder`와 `/author`의 ProfilePage가
   같은 `@id`로 전 페이지 그래프에 실린다. 계약은 `utils/schema/articleAuthor.test.ts`가 고정한다.
