...
- **설계비는 모금 정산(`lib/funding/payout.ts`)과 별개다.** 성공 수수료를 받지 않으므로 정산식에
  이 값을 섞지 않는다. 직접 개설로 되돌리면 행이 지워져 입금 기록도 사라진다(화면이 확인창을 띄운다).
...
- **기록을 먼저 넣고 보낸다.** 크론이 겹쳐도 한 통만 나가게. 발송이 실패하면 기록을 지워
  7일 창 안에서 다음 날 다시 시도하고, 운영자에게 실패 메일이 간다.
- 같은 이메일로 180일 안에 보냈으면 건너뛴다(연습실 단골). 운영자 결제 테스트(`smoke-test`),
  취소·노쇼·환불 건은 대상이 아니다.
