...
## OG 이미지 포맷 규칙 (카카오톡 썸네일)

**`og:image`는 JPEG 또는 PNG만 쓴다. WebP·AVIF 금지.** 카카오톡 스크랩 서버는 WebP를
디코드하지 못해 썸네일뿐 아니라 카드 자체(제목·설명 포함)를 렌더하지 않는다. 페이스북·
슬랙·구글은 WebP를 처리하므로 로컬·다른 채널에서는 정상으로 보여 발견이 늦다 —
검증은 반드시 실제 카카오톡 대화창에서 할 것.

- 정적 OG: `public/images/og-*.jpg` (1200×630). `.gitignore`가 `public/images/**/*.webp`를
  무시하므로 빌드가 만든 `og-*.webp`는 자동으로 제외된다. **예외 규칙을 다시 추가하지 말 것** —
  2026-05 커밋 `397eea2930`이 "포맷이 아니라 404가 원인"이라 오진하고 WebP로 되돌린 탓에
  2026-08까지 전 페이지 카톡 미리보기가 죽어 있었다.
- 스토리 상세: `/api/og/story` 동적 PNG. GET·HEAD 모두 200이어야 한다(스크랩 서버가
  HEAD로 먼저 확인하고 405면 썸네일을 포기).
- 이미지 교체 후 카톡 캐시는 https://developers.kakao.com/tool/debugger/sharing 에서 초기화.
