...
- **저장 형식은 `v1:<iv_b64>:<tag_b64>:<ct_b64>` 한 문자열.** 앞의 `v1`은 장식이 아니라
  나중에 알고리즘이나 키를 바꿀 때 **옛 값을 구분할 유일한 수단**이다. 새 판본을 만들면
  옛 판본을 읽는 경로를 함께 남겨야 한다. IV는 레코드마다 새로 뽑는다(GCM에서 같은 키로
  IV를 재사용하면 기밀성과 인증이 함께 무너진다).
...
- 키가 빠진 배포는 **DB에 흔적을 남기지 않는다.** 원천징수 개설자의 정산 정보 저장이 계좌까지
  통째로 거부되고 정산 기록도 `no_resident_number`로 막히는데, 개설자가 연락해 줄 때까지
  아무도 모른다. 그래서 매일 크론 운영 점검이 키를 왕복으로 찔러 본다
