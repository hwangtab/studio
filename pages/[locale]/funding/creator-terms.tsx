...
export const FUNDING_CREATOR_TERMS_SECTIONS: CreatorTermsSection[] = [
  ...
  },
  {
    ...
    body: [
      '심사에서 승인된 프로젝트도 제목·소개 요약·표지 이미지·본문은 다시 심사를 받지 않고 계속 고칠 수 있습니다. 재심사를 거치지 않는 경로이므로, 고칠 때마다 스튜디오에 알림이 갑니다.',
      ...
      '개설자 계정 정보(소개·연락처·링크)는 프로젝트가 아니라 개설자 계정에 속한 정보이므로, 프로젝트 승인 여부와 무관하게 언제든 고칠 수 있습니다. 다만 이름은 예외입니다 — 승인된 프로젝트가 하나라도 있고 그 이름을 이미 한 번이라도 직접 설정했다면, 그 개설자의 이름은 더 이상 바꿀 수 없습니다. 프로젝트 상세 화면에 판매자 표시와 함께 그대로 노출되는 값이라, 승인 뒤 바뀌지 않는 것들과 같은 이유로 잠급니다. 가입 시 자동으로 채워진 이름을 아직 한 번도 직접 설정하지 않았다면 이 잠금은 적용되지 않습니다.',
    ],
  },
  {
  ...
  },
  {
    ...
    body: [
      '서포터의 개인정보는 스튜디오가 보유하며, 개설자에게 제공하지 않습니다. 리워드의 발송은 개설자가 아니라 스튜디오가 처리합니다.',
      '앞으로 리워드 이행을 위해 서포터의 개인정보를 개설자에게 제공해야 하는 경우가 생기면, 개인정보 처리방침을 개정하고 별도로 고지한 뒤에만 제공합니다. 그 경우에도 개설자는 제공받은 정보를 리워드 제작·배송 목적으로만 사용하며 그 밖의 목적으로 이용하거나 제3자에게 제공할 수 없고, 목적을 다한 정보는 지체 없이 파기해야 합니다.',
    ],
  },
];
