...
export const FUNDING_CREATOR_TERMS_SECTIONS: CreatorTermsSection[] = [
  ...
  },
  {
    ...
    // serializeCreatorTerms()가 그 상수들도 함께 해시한다. 안 그러면 요율만 바꿔도
    // 개설자가 동의한 내용이 판본 그대로 조용히 달라진다.
    ...
    body: [
      ...
      `개설자의 세금 처리 구분이 원천징수인 경우, 수수료를 뺀 금액에서 사업소득 원천징수세액 ${FUNDING_WITHHOLDING_PERCENT}%(소득세 3%, 지방소득세 0.3%)를 떼고 보냅니다. 세금계산서를 발행하는 사업자로 등록한 개설자에게는 원천징수하지 않습니다.`,
      `정산금은 모금이 끝난 날부터 영업일 ${FUNDING_PAYOUT_BUSINESS_DAYS}일 뒤에 보냅니다.`,
      '정산을 받으려면 개설자가 개설자 화면에 은행명·계좌번호·예금주와 세금 처리 구분을 등록해야 합니다. 이 정보는 프로젝트가 승인된 뒤에 등록할 수 있으며, 등록되어 있지 않으면 스튜디오는 정산을 보낼 수 없습니다.',
      '정산 금액을 확정할 때와 실제로 보낼 때 각각 그 내역을 개설자에게 메일로 알립니다. 내역에는 모금액, 환불액, 플랫폼 수수료, 결제 수수료, 원천징수세액, 실지급액과 확정 후원 건수가 담깁니다.',
    ],
  },
  {
