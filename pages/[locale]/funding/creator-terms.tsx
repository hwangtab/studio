...
export const FUNDING_CREATOR_TERMS_SECTIONS: CreatorTermsSection[] = [
  ...
  },
  {
    ...
    body: [
      ...
      '정산을 받으려면 개설자가 개설자 화면에 은행명·계좌번호·예금주와 세금 처리 구분을 모두 등록해야 합니다. 세금 처리 구분이 원천징수인 개설자는 주민등록번호도 함께 등록해야 합니다 — 스튜디오가 원천징수의무자로서 그 세액을 신고하고 국세청에 지급명세서를 제출해야 하는데, 지급명세서에 소득을 받는 분의 주민등록번호가 들어가기 때문입니다. 등록된 번호는 암호화해 보관하고 그 신고·제출 목적으로만 이용하며, 세금 처리 구분을 사업자로 바꿔 저장하면 함께 지워집니다(자세한 사항은 개인정보 처리방침 15항). 사업자로 등록한 개설자에게는 받지 않습니다. 이 정보는 모두 프로젝트가 승인된 뒤에 등록할 수 있습니다.',
