...
export const FUNDING_CREATOR_TERMS_SECTIONS: CreatorTermsSection[] = [
  {
    heading: '제1조 (목적과 당사자)',
    body: [
      '이 약관은 스튜디오 놀(이하 "스튜디오")이 운영하는 펀딩 페이지에 개설자가 직접 프로젝트를 등록하고, 후원자의 리워드 선주문을 받는 절차와 개설자의 의무를 정합니다.',
      '판매자·통신판매업자는 스튜디오 놀입니다. 개설자는 후원자에게 제공하기로 한 리워드의 제작과 이행에 대한 책임을 집니다.',
    ],
  },
  {
    heading: '제2조 (심사)',
  ...
  },
  {
    ...
    body: [
      ...
      '그중에서도 프로젝트 주소(슬러그)·목표 금액·모금 기간(시작일·종료일)은 승인 뒤에는 어떤 경로로도 바뀌지 않습니다. 후원자는 후원 확인 페이지에서 프로젝트 주소로 후원 내역을 찾고, 목표 금액과 모금 기간은 이미 후원한 후원자가 보고 결정한 조건이기 때문입니다.',
      '리워드는 승인과 함께 제목·설명·이미지·금액·수량 제한 여부·배송 필요 여부·예상 전달 시기를 포함해 통째로 잠기며, 승인 뒤에는 어떤 항목도 고치거나 지울 수 없습니다. 설명까지 잠그는 이유는 리워드가 후원자가 보고 결제한 약속이기 때문입니다 — 내용이 바뀌면 후원자 약관 제8조의 "표시·광고와 다르게 이행"에 해당할 수 있습니다.',
  ...
  },
  {
    ...
    body: [
      '모금이 끝나면 스튜디오는 개설자에게 정산금을 보냅니다. 정산금은 후원자가 결제한 후원금에서 환불된 금액을 먼저 뺀 뒤, 그 금액을 기준으로 아래 수수료와 원천징수세액을 뺀 나머지입니다.',
      ...
      '정산을 받으려면 개설자가 개설자 화면에 은행명·계좌번호·예금주와 세금 처리 구분을 모두 등록해야 합니다. 세금 처리 구분이 원천징수인 개설자는 주민등록번호도 함께 등록해야 합니다 — 스튜디오가 원천징수의무자로서 그 세액을 신고하고 국세청에 지급명세서를 제출해야 하는데, 소득세법 시행령 제147조의7제1항제1호 가목이 그 지급명세서에 소득을 받는 분의 주민등록번호를 적도록 하고 있기 때문입니다. 등록된 번호는 암호화해 보관하고 그 신고·제출 목적으로만 이용하며, 세금 처리 구분을 사업자로 바꿔 저장하면 함께 지워집니다. 다만 이미 원천징수한 정산이 있는 경우에는, 그 세액의 신고와 지급명세서 제출 의무가 남아 있으므로 지우지 않고 그 목적으로만 계속 보관합니다(자세한 사항은 개인정보 처리방침 15항). 사업자로 등록한 개설자에게는 받지 않습니다. 이 정보는 모두 프로젝트가 승인된 뒤에 등록할 수 있습니다.',
  ...
  },
  {
    ...
    body: [
      '개설자가 승인된 리워드를 이행하지 못하는 경우, 후원자에게 지급해야 하는 환불의 부담은 개설자에게 있습니다.',
    ],
  },
  {
    heading: '제8조 (후원자 개인정보의 취급)',
    body: [
      '후원자가 후원하며 입력한 개인정보는 스튜디오가 수집·보유합니다. 그중 배송 리워드의 발송에 필요한 항목만, 모금이 마감된 뒤에 개설자에게 제공합니다. 모금 기간 중에는 제공하지 않습니다 — 마감 전에는 후원자가 후원을 취소할 수 있어 받는 분과 주소가 계속 바뀌기 때문입니다.',
      '배송 목록으로 제공하는 항목은 다음뿐입니다: 받는 분 이름, 연락처, 우편번호, 주소, 상세주소, 배송 메모, 그리고 그 후원의 리워드 이름·수량·발송 상태·택배사·운송장번호. 후원자의 이메일 주소, 결제 금액, 결제수단, 주문번호, 응원 메시지는 배송 목록에 담기지 않습니다. 다만 후원자가 명단 공개에 동의한 경우 그 이름과 응원 메시지는 프로젝트 페이지에 공개되므로, 개설자도 그 화면에서 볼 수 있습니다 — 이는 이 조가 말하는 배송 목록 제공과는 다른 경로입니다.',
      ...
      '배송 리워드의 발송 주체는 개설자입니다. 개설자는 개설자 화면에서 후원 건마다 발송 상태(미발송·준비중·발송완료·수령완료)와 택배사·운송장번호를 직접 기록해야 하며, 후원자는 후원 확인 화면에서 그 발송 상태를 보고, 스튜디오는 발송 상태와 함께 택배사·운송장번호까지 봅니다. 택배사·운송장번호는 후원자 화면과 안내 메일에는 나오지 않습니다. 발송 상태를 마지막으로 바꾼 주체가 누구인지는 시스템에 기록으로 남습니다.',
      '후원자에 대한 판매자로서의 계약상 책임은 이 조와 관계없이 스튜디오가 집니다(제1조). 개설자가 이 조를 위반하여 후원자나 제3자에게 손해가 발생한 경우 그 책임은 개설자에게 있으며, 개설자는 스튜디오를 면책합니다.',
    ],
  },
];
