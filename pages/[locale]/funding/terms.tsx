...
export const FUNDING_TERMS_SECTIONS: FundingTermsSection[] = [
  ...
  { heading: '제2조 (정의)', body: ['"프로젝트"는 스튜디오가 제작하는 음반·콘텐츠의 제작비를 모으기 위해 개설한 펀딩 페이지를 말하며, 스튜디오가 심사·승인한 개설자가 운영하는 프로젝트를 포함합니다.', '"개설자"는 스튜디오의 심사를 거쳐 펀딩 페이지에 자신의 프로젝트를 등록하고 리워드의 제작·이행을 맡는 아티스트 등 제3자를 말합니다.', '"리워드"는 후원 금액에 따라 제공하기로 정한 재화 또는 용역(CD, 굿즈, 디지털 음원, 감사 메일 등)입니다.', '"펀딩"은 후원자가 리워드를 선택하고 대금을 결제하여 리워드 선주문 계약을 체결하는 행위입니다.'] },
  { heading: '제3조 (펀딩의 법적 성격)', body: ['펀딩은 전자상거래 등에서의 소비자보호에 관한 법률에 따른 통신판매 계약이며, 기부가 아닙니다.', '후원자는 후원 금액에 대응하는 리워드를 받습니다. 반대급부 없이 금품을 받는 것이 아니므로 기부금품의 모집 및 사용에 관한 법률에 따른 기부금품 모집에 해당하지 않습니다.', '스튜디오는 기부금영수증을 발급하지 않으며 후원 금액은 세액공제 대상이 아닙니다. 페이지의 "펀딩"·"응원" 표현은 리워드 선주문을 뜻합니다.'] },
  ...
  { heading: '제6조 (후원 금액의 집행)', body: ['프로젝트는 Keep-it-All 방식입니다. 목표 금액에 미달하더라도 모금액으로 제작을 진행하며, 목표 미달을 이유로 펀딩이 취소되지 않습니다.', '제작이 불가능해진 경우 스튜디오는 후원자에게 고지하고 후원 금액 전액을 환불합니다.'] },
  { heading: '제7조 (리워드의 제공)', body: ['리워드의 예상 전달 시기는 제작 계획에 따른 예정이며, 지연이 예상되면 스튜디오는 후원자에게 이메일로 고지합니다.', '배송 리워드의 발송은 스튜디오가 처리하며, 후원자의 배송지는 개설자에게 제공되지 않습니다. 배송지 오류로 인한 반송·재발송 비용은 후원자가 부담합니다.', '개설자가 있는 프로젝트는 리워드의 제작을 개설자가 수행할 수 있습니다. 이 경우에도 후원자에 대한 계약상 책임(리워드 미이행에 따른 환불 등)은 판매자인 스튜디오가 집니다. 환불·청약철회의 접수 창구는 개설자 유무와 관계없이 이 약관에 정한 대로 스튜디오입니다.'] },
  { heading: '제8조 (청약철회의 권리 및 기간)', body: ['후원자는 프로젝트 마감 전이고 리워드 발송 준비가 시작되기 전이면 언제든 펀딩을 취소하고 전액 환불받을 수 있습니다. 온라인 결제는 후원 확인 페이지에서 바로 취소되며, 스튜디오가 계좌로 받아 등록한 펀딩은 문의로 접수하면 계좌로 환불해 드립니다.', '음원·영상 등 내려받는 형태의 리워드는 내려받기가 시작된 뒤에는 청약철회가 제한됩니다(전자상거래법 제17조 2항 5호). 내려받기 전이라면 위 기간 안에 언제든 철회할 수 있습니다.', '리워드를 받은 날부터 7일 이내에 청약철회할 수 있습니다. 리워드가 표시·광고 내용과 다르거나 계약 내용과 다르게 이행된 경우 리워드를 받은 날부터 3개월 이내, 그 사실을 안 날부터 30일 이내에 청약철회할 수 있습니다.'] },
  ...
  { heading: '제13조 (개인정보의 처리)', body: [`스튜디오는 펀딩 확정·리워드 발송·고객 응대 목적으로 후원자의 이름·연락처·이메일·배송지를 수집하며, ${PRIVACY_RETENTION_TEXT} 보관한 뒤 파기합니다. ${PRIVACY_LEGAL_RETENTION_TEXT}`, '후원자 명단 공개에 동의한 후원자의 이름과 응원 메시지는 프로젝트 페이지에 표시되며, 동의는 후원 확인 페이지 또는 문의로 철회할 수 있습니다. 스튜디오는 타인의 권리를 침해하거나 프로젝트와 무관한 내용의 메시지를 표시에서 제외할 수 있습니다.', `수집 항목·이용 목적·보유 기간·처리위탁 현황은 ${FUNDING_PRIVACY_SECTION_REFERENCE}에 함께 고지합니다.`] },
