...
export const FUNDING_TERMS_SECTIONS: FundingTermsSection[] = [
  ...
  { heading: '제2조 (정의)', body: ['"프로젝트"는 스튜디오가 제작하는 음반·콘텐츠의 제작비를 모으기 위해 개설한 펀딩 페이지를 말합니다.', '"리워드"는 펀딩 금액에 따라 스튜디오가 제공하기로 정한 재화 또는 용역(CD, 굿즈, 디지털 음원, 감사 메일 등)입니다.', '"펀딩"은 서포터가 리워드를 선택하고 대금을 결제하여 리워드 선주문 계약을 체결하는 행위입니다.'] },
  ...
  { heading: '제7조 (리워드의 제공)', body: ['리워드의 예상 전달 시기는 제작 계획에 따른 예정이며, 지연이 예상되면 스튜디오는 서포터에게 이메일로 고지합니다.', '배송 리워드는 서포터가 입력한 배송지로 발송하며, 배송지 오류로 인한 반송·재발송 비용은 서포터가 부담합니다.'] },
