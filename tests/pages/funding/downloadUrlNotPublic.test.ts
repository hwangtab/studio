...
/**
 * 리워드 `downloadUrl`은 후원자에게만 가야 한다.
 *
...
describe('내려받기 주소는 공개 화면 props에 실리지 않는다', () => {
  ...
  it('벗기는 함수가 모든 리워드의 주소를 지운다', () => {
    const withUrl = {
      ...live[0],
      rewards: [{ ...live[0].rewards[0], downloadUrl: 'https://cdn.example/a.zip' }],
    };
    expect(stripRewardDownloads(withUrl).rewards.every((r) => r.downloadUrl === null)).toBe(true);
  });

  it('상세 페이지 props 어디에도 주소가 없다', async () => {
    // 내려받기 주소를 실제로 가진 프로젝트로만 본다 — 없으면 이 테스트가 아무것도 안 지킨다.
    const target = live.find((p) => p.rewards.some((r) => r.downloadUrl));
    ...
    for (const r of getFundingProject(target!.slug)!.rewards) {
      if (r.downloadUrl) expect(serialized).not.toContain(r.downloadUrl);
    }
  ...
  it('서버는 원본 값을 그대로 본다 — 메일·후원 확인 페이지가 그 값을 쓴다', () => {
    const target = live.find((p) => p.rewards.some((r) => r.downloadUrl));
    expect(getFundingProject(target!.slug)!.rewards.some((r) => r.downloadUrl)).toBe(true);
  });
});
