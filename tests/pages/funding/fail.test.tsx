...
const run = async (query: Record<string, string>) =>
  (await getServerSideProps({ params: { locale: 'ko' }, query, res: resStub() } as never)) as { props: { slug: string | null; code: string | null; message: string } };
...
it('정본 전화번호를 상시 표기한다', () => {
  render(<FundingFailPage slug="demo" code={null} message="결제창이 닫혔거나 결제가 거절되었습니다." />);
...
it('코드가 있으면 문의용으로 함께 보여준다', () => {
  render(<FundingFailPage slug={null} code="REJECT_CARD_COMPANY" message="카드사에서 결제를 거절했습니다." />);
