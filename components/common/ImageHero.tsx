    ...
    >
      ...
      <div className="absolute inset-0 z-0 hero-zoom">
        <ResponsiveImage
          ...
          height={1080}
          sizes="100vw"
          // hero 배경 이미지는 어두운 그라디언트 오버레이 위에 깔리고 줌 애니메이션
          // 중에 보이므로 quality 70으로 충분 (기본 75 대비 ~7-10% 용량 감소,
          // 모바일 LCP 영향 작음). 목표: hero 이미지 전송량 추가 감축.
          quality={70}
        />
      </div>
      ...
      <div className={`container mx-auto px-4 z-20 relative ${alignmentClass}`}>
        ...
        <div>
          ...
          {subtitle && (
            <div
              className={`font-pretendard text-xl md:text-3xl text-gray-200 mb-10 max-w-2xl leading-relaxed opacity-90 ${textBreakClass} ${textAlign === 'center' ? 'mx-auto' : ''}`}
            >
              {subtitle}
            </div>
          )}
