...
const Playlist = ({ tracks, currentTrackIndex, isPlaying, onSelectTrack }: PlaylistProps) => {
    return (
        <div className="h-full overflow-y-auto pr-2 custom-scrollbar">
            <h3 className="text-white/60 text-xs font-bold uppercase tracking-wider mb-4 px-2">
                ...
                {tracks.map((track, index) => {
                    ...
                    return (
                        <motion.div
                            ...
                            className={`group flex items-center p-3 rounded-lg cursor-pointer transition-colors border border-transparent ${isActive ? 'border-primary/30' : 'hover:border-white/5'
                                }`}
                            onClick={() => onSelectTrack(index)}
                        >
                            <div className="relative w-10 h-10 rounded overflow-hidden flex-shrink-0 mr-4">
                                <img
                                    src={track.albumArt}
                                    alt={track.title}
