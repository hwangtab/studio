...
const PlayerControls = ({
    ...
    return (
        <div className="flex items-center justify-center gap-6">
            <m.button
                whileHover={shouldAnimate ? { scale: 1.1, color: '#fff' } : undefined}
            ...
            <m.button
                whileHover={shouldAnimate ? { scale: 1.05, boxShadow: "0 0 20px rgba(var(--primary-rgb), 0.5)" } : undefined}
                whileTap={shouldAnimate ? { scale: 0.95 } : undefined}
                onClick={onPlayPause}
            ...
            >
                <div className={`absolute inset-0 rounded-full border border-gray-900/50 dark:border-white/50 opacity-0 group-hover:opacity-100 ${shouldAnimate ? 'animate-ping-slow' : ''}`} />
