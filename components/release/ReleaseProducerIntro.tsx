    ...
    <div className="max-w-3xl mx-auto">
      <div className="glass-card rounded-3xl p-8 sm:p-10 text-center">
        <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-primary/10 mb-5">
          <Award size={32} className="text-primary dark:text-primary-lighter" />
        </div>
        <h2 className="text-2xl sm:text-3xl font-title font-bold text-gray-900 dark:text-white mb-2">
          {sectionTitle}
        </h2>
        <p className="text-sm text-primary dark:text-primary-lighter font-medium mb-6">
          {tagline}
        </p>
          ...
          <div className="grid grid-cols-3 gap-4 pt-6 border-t border-gray-100 dark:border-gray-700">
            {stats.map((stat, index) => (
              <div key={`${stat.value}-${index}`}>
                <p className="text-2xl sm:text-3xl font-bold text-primary dark:text-primary-lighter mb-1">{stat.value}</p>
