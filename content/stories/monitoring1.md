...
faq:
  ...
  - q: 한쪽 귀에만 헤드폰을 끼고 녹음하는 게 좋은가요?
    a: >-
      ...
      수 있어 어색함이 줄지만 음정 모니터링이 어려울 수 있습니다. 초보자에게는 두 쪽 착용을 권합니다.
howTo:
  name: "보컬 녹음 헤드폰 모니터링 큐 믹스 설정 단계"
  description: "보컬 녹음에서 음정 안정과 편안한 퍼포먼스를 위한 헤드폰 큐 믹스 4단계 설정 절차입니다."
  steps:
    - name: "1단계: 본인 목소리 볼륨"
      text: "본인 목소리는 MR 대비 +3~5dB 높게 설정합니다. 너무 작으면 음정 판단이 어렵고, 너무 크면 과발성으로 이어집니다."
    - name: "2단계: 리버브 추가"
      text: "Pre-delay 20~30ms로 소량의 모니터링용 리버브를 추가합니다. 건조한 목소리의 어색함을 줄이지만 과하면 음정 이탈을 감추므로 주의합니다."
    - name: "3단계: MR 볼륨 균형"
      text: "MR이 너무 크면 본인 목소리를 듣지 못해 음정 불안정, 너무 작으면 박자·하모니 유지가 어렵습니다. 편안한 균형점을 찾습니다."
    - name: "4단계: 레이턴시 제거"
      text: "오디오 인터페이스의 Direct Monitoring 기능을 활성화해 DAW를 거치지 않고 직접 모니터링합니다. 15ms+ 레이턴시는 음정 불안의 주요 원인입니다."
---
...
최적의 모니터링 환경은 보컬리스트가 자신의 소리에 집중하게 해줍니다. 스튜디오 놀의 전문 모니터링 시스템에서 편안한 큐 믹스를 세팅해 좋은 보컬 퍼포먼스를 경험하세요.
...
큐 믹스에서 본인 보컬 볼륨이 MR 대비 +3~5dB 높게 설정되면 음정을 가장 명확하게 모니터링할 수 있으며, 이 비율이 흔들리면 녹음 중 음정 불안정과 과발성으로 이어지는 경우가 많습니다. 이어 피로가 쌓인 상태에서 계속 녹음하면 음정 판단력이 떨어져 테이크 수만 늘어나는 악순환이 생깁니다. 30분 녹음 후 5분 완전 정적 휴식을 지키는 것이 세션 전체의 완성도를 높이는 가장 단순하고 효과적인 방법입니다.

## 노래가 안 풀리는 이유가 목이 아니라 헤드폰일 때가 많습니다

녹음 중에 자꾸 음정이 흔들리는 분을 보면, 저는 발성부터 의심하지 않습니다. 헤드폰을 먼저 의심합니다. 십중팔구 큐 믹스 문제거든요. MR에 자기 목소리가 파묻혀 있으면 사람은 무의식적으로 더 크게 지르게 되고, 그러면 음정이 위로 뜹니다. 그래서 "제 목소리가 잘 안 들려요"라는 말이 나오기 전에 보컬을 MR보다 살짝 앞으로 당겨 두는 게 제 첫 세팅입니다.

두 번째로 신경 쓰는 건 드라이한 목소리가 주는 어색함입니다. 아무 효과 없이 자기 생목소리를 헤드폰으로 들으면 대부분 위축됩니다. 그래서 녹음 경로에는 손대지 않고 모니터로 나가는 소리에만 짧은 리버브를 살짝 얹어 드립니다. 무대에서 부르는 느낌이 나면 표현이 확실히 과감해지거든요. 이건 녹음되는 파일에는 전혀 들어가지 않으니 마음 놓고 쓰셔도 됩니다.

그리고 세션이 길어지면 저는 먼저 헤드폰 볼륨을 낮추자고 제안합니다. 큰 소리로 오래 들으면 귀가 지쳐서 음정 판단이 무뎌지고, 그때부터는 테이크만 쌓이고 쓸 만한 게 안 나옵니다. 30분 부르고 5분은 완전히 조용히 쉬는 편이 결국 더 빨리 끝나요. 급해 보여도 이게 제일 빠른 길입니다.

사실 이런 편안한 모니터링 세팅은 홈레코딩에서 가장 재현하기 어려운 부분이기도 합니다. 제대로 잡힌 환경에서 녹음부터 발매까지 함께 진행하고 싶다면 [발매 프로젝트](/release-project) 안내를 참고하세요.

---

[첫 녹음 세션 준비 가이드](/stories/session1) | [보컬 녹음 실수 10가지](/stories/mistakes1) | [홈 레코딩 첫 장비 구입 가이드](/stories/homegear1) | [보컬 EQ 완전 가이드](/stories/eq1) | [홈 레코딩 노이즈 제거 가이드](/stories/noise-reduction1) | [마이크 선택 완전 가이드](/stories/microphone-types1)
