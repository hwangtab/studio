...
faq:
  ...
  - q: Spotify 알고리즘이 선호하는 곡 길이는?
    a: >-
      2분 30초~3분 30초가 Spotify 알고리즘에서 완청률이 가장 높은 구간입니다. 30초 이내에 후크가 등장해야 스킵 없이 들을
      확률이 높아집니다. Discover Weekly, Release Radar 등 개인화 플레이리스트는 완청률을 핵심 지표로 활용합니다.
  - q: 음원 발매 시 가장 먼저 해야 할 것은?
    a: >-
      발매 28일 전(최소 7일 전)에 Spotify for Artists에서 에디토리얼 피칭을 제출하는 것이 최우선입니다. 동시에
      사전 저장(pre-save) 링크를 만들어 SNS에 배포하고, 유통사를 통해 국내 플랫폼(멜론·지니) 플레이리스트 피칭도 요청합니다.
---
...
스튜디오 놀 발매 클라이언트들이 스트리밍 노출 확대에서 자주 효과를 본 영역입니다.
...
## 알고리즘은 결국 '끝까지 들었나, 스킵했나'를 보고 있어요

스트리밍 최적화라고 하면 다들 피칭이나 태그 이야기부터 하는데, 저는 곡 자체의 구조부터 봐요. 스포티파이의 개인화 추천은 사람이 이 곡을 처음부터 끝까지 들었는지, 아니면 30초쯤에 넘겨버렸는지를 가장 강한 신호로 읽거든요. 그래서 제일 먼저 손보는 게 인트로예요. 15초 안에 후크나 인상적인 한 소절이 나와야 스킵을 막아요. 잔잔한 앰비언스로 40초를 끄는 편곡은 감성적으로는 멋져도, 알고리즘 관점에선 청자를 놓치는 구간이에요. 곡 길이도 2분 30초에서 3분 30초 사이가 완청률이 제일 잘 나오는 구간이라, 특별한 의도가 없다면 이 안에 담는 걸 권해요.

두 번째는 소리의 크기예요. 마스터를 -14 LUFS, 트루 피크 -1dBTP 이하로 맞춰야 해요. 이걸 안 지키면 스포티파이가 자동으로 음량을 깎는데, 그러면 플레이리스트에서 내 곡만 유독 작고 밋밋하게 들려서 무의식중에 스킵당해요. 완청률 싸움에서 스스로 불리한 조건을 지고 시작하는 셈이에요.

세 번째는 발매 후 첫 72시간이에요. 이 구간에 알고리즘이 소규모로 곡을 테스트 노출하면서 반응을 보거든요. 그러니 팬들에게 '재생만 하지 말고 저장하고 플레이리스트에 담아달라'를 구체적으로 요청하세요. 저장은 단순 재생보다 훨씬 강한 신호라, 이게 쌓이면 테스트 배포가 확대돼요. 곡 구조·음압·초기 반응, 이 세 가지가 맞물릴 때 알고리즘이 곡을 밀어주기 시작해요. 발매 초기 며칠을 어떻게 설계하는지는 [스트리밍 알고리즘 완전 가이드](/stories/streaming-algorithm1)에서 더 깊이 이어집니다.
