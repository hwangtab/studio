...
## AI 마스터링은 도구의 한계를 알고 쓰면 꽤 쓸 만해요

AI 마스터링을 두고 '이거 써도 되나요'라고 물으시는 분들이 많은데, 저는 목적만 맞으면 얼마든지 쓰라고 해요. LANDR 같은 서비스는 수십만 곡을 학습해서 스펙트럼과 음압 목표값을 자동으로 맞춰 주거든요. SNS 올릴 데모, 홈 레코딩 습작, 발매 전 임시 확인용이라면 몇 분 만에 그럴듯한 결과가 나와요. 다만 알고리즘한테 곡의 감정이나 의도를 기대하면 안 돼요. AI는 '이 장르 평균은 이렇더라'를 재현할 뿐, 이 곡만의 다이나믹을 살리거나 믹스에 숨은 문제를 진단하진 못하거든요.

도구를 제대로 쓰려면 입력을 깨끗하게 넣어야 해요. 믹스에 이미 리미터가 걸린 채로 AI에 던지면, AI가 그 위에 또 리미터를 얹어서 '이중 리미팅'이 나요. 트랜지언트가 뭉개지고 다이나믹이 죽죠. 마스터 버스 리미터는 빼고, 헤드룸 -3~-6dBFS, True Peak -1dBTP 이내로 넣어 주세요. Intensity는 발라드·재즈처럼 여백이 중요한 곡은 Low, 팝·록은 Medium이 무난하고요.

결과를 판단할 때 꼭 지켜야 할 게 하나 있어요. AI 결과랑 전문 마스터링을 비교할 땐 반드시 음량을 똑같이 맞춰 놓고 들으세요. 둘 다 -14 LUFS로 정규화한 뒤 들어야 '더 크니까 더 좋게 들리는' 착각 없이 음색과 다이나믹 차이가 제대로 보여요. 이렇게 비교해 보고도 아쉬움이 남는다면, 그때가 [전문 마스터링](/stories/mastering1)으로 넘어갈 타이밍이에요.
