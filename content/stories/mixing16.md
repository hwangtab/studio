...
summary: >-
  200Hz 이하 하이패스, 1~3kHz 부스트로 보컬 선명도 향상. 서브트랙티브 우선, 애디티브 보완 EQ 순서.
thumbnail: "/images/recording16.webp"
faq:
  - q: "EQ(이퀄라이저)란 무엇인가요?"
    a: "EQ는 소리의 특정 주파수 대역을 깎거나 키워주는 도구입니다. 믹싱에서 악기 간 주파수 충돌을 정리하고, 보컬을 선명하게 만드는 데 주로 사용됩니다."
  - q: "보컬 EQ는 어떻게 설정하나요?"
  ...
  - q: "서브트랙티브 EQ와 애디티브 EQ의 차이는 무엇인가요?"
    a: "서브트랙티브 EQ는 불필요한 주파수를 깎는 방식이고, 애디티브 EQ는 원하는 주파수를 올리는 방식입니다. 믹싱에서는 먼저 서브트랙티브로 문제를 해결하고, 필요한 경우에만 애디티브를 사용하는 순서가 좋습니다."
inlineFallback:
...
초보자들이 자주 하는 실수는 EQ로 자꾸 뭔가를 **'키우려고(Boost)'** 하는 것입니다. 하지만 진짜 고수들은 **'깎는(Cut)'** 것부터 시작합니다. (비우기!)
...
## Studio NOL 보컬 EQ 표준 3가지

스튜디오 놀 보컬 믹싱 의뢰에서 일관되게 적용하는 EQ 표준입니다.

**1. 200Hz 이하 하이패스 — 룸 노이즈·저역 정리**

보컬 첫 EQ 작업은 항상 200Hz 이하 하이패스로 룸 노이즈·저역 정리부터 시작합니다. 보컬리스트에 따라 100~250Hz 사이에서 자연스러운 지점을 찾습니다.

**2. 200~400Hz 박스 사운드 -2~3dB 컷**

이 대역에 보컬 박스 사운드가 쌓이는 경우가 많습니다. -2~3dB 컷으로 명료도가 살아납니다. 너무 많이 컷하면 보컬 두께감이 사라집니다.

**3. 1~3kHz 부스트 + 10kHz 에어**

1~3kHz 살짝 부스트(+1~2dB)로 명료도 강조, 10kHz 셸프 부스트(+1~2dB)로 에어감 추가. 보컬리스트별로 거슬리는 피크는 컷으로 처리합니다.
...
**믹싱 강좌 시리즈**: [← 제15부: 좌우의 미학, 패닝(Panning)](/stories/mixing15) | [제17부: 음악의 생동감, 컴프레서 →](/stories/mixing17) | [📚 믹싱 완전 가이드 (전체 로드맵)](/stories/mixing-complete-guide)
