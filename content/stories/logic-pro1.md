...
## 저는 로직 내장 플러그인만으로 데모를 끝내라고 합니다

15년 동안 온갖 홈 레코딩 소스를 받아봤지만, 로직 쓰는 분들이 가장 자주 하는 실수는 값비싼 서드파티 플러그인부터 사려는 겁니다. 저는 반대로 말립니다. 위 보컬 체인(Channel EQ → Vintage VCA → DeEsser → ChromaVerb)만 손에 익혀도 데모는 물론이고 웬만한 발매 직전 단계까지 로직 안에서 다 끝납니다. 플러그인을 더 사는 것보다 지금 있는 다섯 개를 정확히 이해하는 편이 결과물을 훨씬 크게 바꿉니다.

Flex Pitch는 특히 '얼마나 안 고치느냐'가 실력입니다. 드래그 한 번으로 음을 딱 맞춰버리고 싶은 유혹이 큰데, 그렇게 전 구간을 100%로 당기면 사람 목소리가 아니라 신디사이저가 됩니다. 본문에 적은 대로 편차의 절반쯤만 당기고, 음 끝의 비브라토와 시작음의 슬라이드는 손대지 않는 편이 자연스럽습니다. 저도 세션에서 Flex Pitch를 열면 오히려 '건드리지 않을 음'부터 먼저 골라둡니다.

Mastering Assistant는 편리하지만 정답으로 받아들이진 마시라고 합니다. 제안값은 출발점일 뿐이고, 곡마다 저역이 뭉친 정도나 보컬 존재감이 다르기 때문에 그 위에서 손으로 다시 조정해야 합니다. 어디까지나 '내 곡이 지금 어떻게 들리는지' 감을 잡는 용도로 쓰시고, 정식 발매까지 생각한다면 홈에서 뽑은 마스터를 최종본이라 여기지 마세요. 그 이후 과정은 [발매 프로젝트](/release-project)에서 따로 짚습니다.
