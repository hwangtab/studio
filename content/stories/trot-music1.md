...
summary: >-
  트로트 음악 제작 과정을 작곡·편곡·녹음 흐름에 맞춰 정리합니다. 홈레코딩에서 적용할 방법과 Studio NOL 제작 상담 전 준비할 내용을 함께 확인하세요.
faq:
  ...
  - q: 트로트 녹음 시 가장 중요한 점은?
    a: >-
      보컬의 즉흥적 장식음(임프로비제이션)을 충분히 여러 테이크 녹음해 가장 좋은 감성 표현을 선택(컴핑)하는 것이 중요합니다. 트로트 보컬
...
**녹음 방법**
- 여러 테이크 녹음 후 가장 좋은 장식음 순간 선택 (컴핑)
...
## 트로트는 장식음을 살리느냐 지우느냐로 갈려요

트로트 프로덕션을 팝처럼 다루면 트로트가 사라져요. 팝 보컬은 음정의 정확성과 음색의 균일함을 좇지만, 트로트의 생명은 그 반대편, 즉 음을 꾸미는 장식음에 있거든요. 목표 음을 아래에서 밀어 올리거나 위에서 흘러내리는 포르타멘토, 음 끝에 떨리는 비브라토, 성문을 순간 조이는 코드(聲門)—이 흔들림들이 한(恨)과 흥(興)을 만듭니다. 그래서 녹음·믹싱 내내 제 머릿속 원칙은 하나예요. "이 장식음을 지우지 말 것."

구체적으로는 자동 피치 교정을 최소로 두세요. Auto-Tune이나 Melodyne으로 포르타멘토를 반듯하게 펴 버리면 그 처연한 미끄럼이 통째로 사라져요. 여러 테이크를 넉넉히 받아 가장 감정이 실린 장식음 순간을 컴핑으로 골라내는 게 훨씬 낫습니다. 컴프레서도 세게 물리지 마세요. 팝은 일관된 레벨을 위해 강하게 눌러도 되지만, 트로트에서 Ratio 2:1~3:1보다 더 조이면 장식음의 다이나믹이 뭉개져요.

음색은 따뜻함이 핵심이에요. 흉성 공명이 모이는 중저역(200~400Hz)을 깎지 말고 살려 두면 트로트 특유의 두껍고 정 있는 목소리가 남습니다. 고역은 팝만큼 세게 밀 필요 없고요. 리버브는 플레이트 타입에 Decay 1~1.5초 정도가 그 감성에 잘 붙어요. 세대를 아우르는 장르인 만큼 가사가 또박또박 들리도록 딕션을 살리는 것도 놓치지 마세요. 여러 테이크에서 최선을 골라내는 컴핑 감각은 [오디션 데모 녹음 가이드](/stories/demo-recording1)에서도 이어집니다.
