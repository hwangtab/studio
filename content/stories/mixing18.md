...
faq:
  - q: "리버브(Reverb)란 무엇이고 믹싱에서 어떻게 사용하나요?"
    a: "리버브는 소리가 공간에 반사되는 잔향 효과를 시뮬레이션하는 도구입니다. 믹싱에서는 악기나 보컬에 공간감을 더해 사운드에 입체감을 부여합니다. 보통 센드/리턴 방식으로 하나의 리버브를 여러 악기가 공유하는 방법을 권합니다."
  ...
  - q: "보컬에 리버브를 너무 많이 넣으면 어떻게 되나요?"
    a: "과도한 리버브는 보컬이 뭉개져 가사 전달력이 떨어집니다. 리버브량은 믹스 전체 볼륨을 기준으로 조절하고, 필요하면 딜레이를 먼저 사용하여 자연스러운 공간감을 만드는 방법을 권합니다."
inlineFallback:
  price: lesson-monthly
howTo:
  name: "믹싱 리버브 설정 4단계 절차"
  description: "장르에 맞는 리버브 종류 선택부터 EQ 처리·양 조절까지 자연스러운 공간감을 만드는 절차입니다."
  steps:
    - name: "1단계: 장르에 맞는 리버브 종류 선택"
      text: "팝 보컬은 Plate(밝고 밀도 높음), 발라드·웅장한 사운드는 Hall(길고 넓음), 드럼·리듬 기타는 Room(짧고 현실적)을 선택합니다."
    - name: "2단계: 프리딜레이 설정으로 명료도 확보"
      text: "리버브 Pre-delay를 40~80ms로 설정합니다. 원음이 먼저 선명하게 들리고 잔향이 뒤따르므로, 보컬이 뭉개지지 않으면서 공간감을 유지할 수 있습니다."
    - name: "3단계: Abbey Road Trick — 리버브 EQ 적용"
      text: "리버브 채널에 EQ를 걸어 Low Cut 600Hz 이하, High Cut 6~10kHz 이상을 잘라냅니다. 저음 잔향이 베이스와 뭉개지는 것과 고음 치찰음이 귀를 찌르는 것을 동시에 방지합니다."
    - name: "4단계: 리버브 양 최소화 — 들릴락 말락이 최적"
      text: "리버브 페이더를 낮춰 '리버브가 있나?' 싶을 정도로 줄입니다. 그 상태에서 뮤트했을 때 확연히 건조해지면 올바른 양입니다. 과한 리버브는 보컬 전달력을 약화시킵니다."
---
...
오늘은 믹싱의 꽃이자, 초보자가 자주 실수하는 영역인 '공간의 미학', 리버브에 대해 깊이 파고들어 보겠습니다.
...
## Studio NOL 보컬 믹싱에서 리버브를 결정하는 3가지 기준

스튜디오 놀 보컬 믹싱 의뢰에서 리버브 종류·양·Pre-delay를 결정하는 반복 기준입니다.

**1. 장르별 리버브 타입**

K-POP·팝 보컬은 플레이트 리버브(Decay 0.8~1.2초)로 밀도감과 광택. 발라드 보컬은 홀 리버브(Decay 1.5~2.5초)로 넓은 공간감. 록 보컬은 룸 리버브(Decay 0.6~1.0초)로 라이브 무대 느낌. 한 곡에 두 종류 이상의 리버브를 Aux로 병렬 운용하는 듀얼 레이어링도 자주 쓰입니다.

**2. Pre-delay — BPM 기반 계산**

Pre-delay를 BPM 기준 16분음표 길이(60,000÷BPM÷4)로 설정하면 리버브가 비트와 자연스럽게 정렬됩니다. 120BPM이면 Pre-delay 125ms 또는 절반인 62.5ms. 이 단순한 계산만으로 리버브가 훨씬 음악적으로 들립니다.

**3. 리버브 EQ — 100Hz 하이패스 필수**

리버브 Aux 버스에 100Hz 하이패스 필터를 적용해 저역 에너지가 리버브 테일에 쌓이는 탁함을 방지합니다. 8kHz 이상도 -2~4dB 살짝 컷하면 리버브가 자연스럽게 공간 안으로 스며듭니다.

---

**믹싱 강좌 시리즈**: [← 제17부: 음악의 생동감, 컴프레서](/stories/mixing17) | [제19부: 소리의 그림자, 딜레이(Delay) →](/stories/mixing19) | [📚 믹싱 완전 가이드 (전체 로드맵)](/stories/mixing-complete-guide)
