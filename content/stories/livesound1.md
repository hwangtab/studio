...
author: "스튜디오 놀"
category: "가이드"
tags: ["라이브 공연 음향", "PA 시스템", "모니터 스피커", "라이브 보컬", "공연 음향 기초", "인이어 모니터", "라이브 믹싱"]
thumbnail: "/images/recording11.webp"
summary: "라이브 공연에서 보컬리스트가 알아야 할 PA 시스템 기초, 모니터 스피커 활용법, 사운드 체크 방법, 인이어 모니터 설정 팁을 정리합니다."
faq:
  - q: "PA 시스템이란 무엇인가요?"
    a: "PA(Public Address) 시스템은 마이크 → 믹서 → 앰프 → 스피커로 이어지는 공연 음향 증폭 시스템입니다. 보컬, 악기 소리를 관객에게 전달하는 메인 스피커 시스템과 연주자가 자신의 소리를 듣기 위한 모니터 시스템으로 나뉩니다."
  - q: "사운드 체크는 어떻게 하나요?"
    a: "공연 전 30분~1시간 전에 진행합니다. 마이크 레벨 확인 → 모니터 볼륨 조정 → EQ 설정 → 전체 밸런스 확인 순서로 진행합니다. 사운드 엔지니어에게 '모니터에 보컬 더 올려주세요', '베이스 좀 줄여주세요' 등 명확하게 요청하세요."
  - q: "인이어 모니터와 웨지 모니터의 차이는 무엇인가요?"
    a: "웨지(Wedge) 모니터는 무대 앞에 놓는 스피커이고, 인이어 모니터(IEM)는 귀에 꽂는 이어폰 형태입니다. 인이어는 소음 차단이 좋고 일관된 사운드를 제공하지만 귀 건강 주의가 필요합니다. 소규모 공연에서는 웨지 모니터가 일반적입니다."
  - q: "마이크 피드백(하울링)은 왜 생기고 어떻게 막나요?"
    a: "마이크가 스피커 앞에 있을 때 소리가 마이크→스피커→마이크로 무한 루프되어 발생합니다. 마이크를 스피커 뒤에 위치시키고, 볼륨을 적정 수준 이하로 유지하면 대부분 해결됩니다. 사운드 엔지니어가 EQ로 공진 주파수를 줄이기도 합니다."
---
