...
## 리하모니제이션은 같은 멜로디에 다른 화성의 옷을 입히는 일이에요

리하모니제이션, 화성 재창조는 멜로디는 그대로 두고 그 아래 코드를 바꿔 곡의 분위기를 새로 칠하는 작업이에요. 가장 보편적인 도구가 트라이톤 대체예요. 도미넌트 코드(V7)를 그 트라이톤(♭5) 거리에 있는 다른 도미넌트로 바꿔치기하는 건데, 베이스가 반음씩 미끄러져 내려오는 그 세련된 흐름이 밋밋한 진행에 재즈 색을 확 입혀 줘요. G7을 D♭7로 바꿔 보면 그 느낌이 바로 와요.

또 하나 유용한 게 세컨더리 도미넌트예요. 코드와 코드 사이에 '다음 코드를 향한 임시 도미넌트'를 하나 끼워 넣으면, 진행에 당기는 힘이 생겨서 더 그럴듯하게 흘러가요. 밋밋하게 이어지던 두 코드 사이에 이 징검다리 하나만 놓아도 곡이 살아나요.

다만 명심할 게, 이론적으로 가능한 대체가 다 좋은 건 아니에요. 아무리 멋진 대체 코드도 그 곡의 분위기와 어긋나면 오히려 어색해요. 그래서 반드시 원곡의 원래 화성과 내가 바꾼 버전을 번갈아 쳐 보며 귀로 비교해야 해요. 결국 판단은 이론이 아니라 귀가 하는 거예요. 이 비교 청취는 미묘한 화성 색차를 들어야 해서 조용한 공간이 필요해요. 스튜디오 놀 방음 개인실에서 한 곡을 여러 화성으로 갈아입혀 보세요. 개별 코드를 다른 코드로 바꾸는 대체 기법은 [피아노 코드 대체 연습](/stories/practice-room-piano-chord-substitution1)에서 이어집니다.
