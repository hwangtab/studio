...
summary: >-
  믹싱할 때 레퍼런스 트랙 쓰는 법 설정 기준과 작업 순서를 정리합니다. 홈레코딩에서 바로 적용할 체크포인트와 Studio NOL 믹싱 의뢰 전 준비할 파일까지 확인하세요.
faq:
...
## Studio NOL 보컬 믹싱에서 레퍼런스 곡을 활용하는 3가지 방식

스튜디오 놀 보컬 믹싱 의뢰에서 레퍼런스 곡이 결과 방향을 결정하는 반복 활용 방식입니다.

**1. 의뢰 단계 — 레퍼런스 1~3곡 명확화**

"느낌대로 해주세요"는 가장 흔한 NG 패턴입니다. 의뢰 단계에서 보컬 위치(앞/뒤)·공간감(드라이/웻)·음색(밝은/어두운) 기준으로 1~3곡씩 레퍼런스를 공유하면 첫 믹스가 목표에 훨씬 가까워져 수정 라운드가 줄어듭니다.

**2. 작업 중 A/B — 같은 레벨로 비교**

레퍼런스 곡과 작업 중인 믹스를 같은 레벨(LUFS)로 맞춘 뒤 A/B 비교합니다. 레퍼런스가 -10 LUFS인데 작업물이 -18 LUFS면 단순히 "더 크게" 들려 비교가 무의미합니다. 라우드니스 정렬 플러그인 또는 수동 게인 조정으로 동일 레벨에서 비교해야 객관적 판단이 가능합니다.

**3. 한계 인식 — 장비·재료 차이**

상업 음원 레퍼런스를 같은 수준으로 만들려면 그만큼의 재료(연주·녹음 품질)와 시간(수십~수백 시간 믹싱)이 필요합니다. 의뢰 단계에서 "이 곡 같은 사운드"의 기대치를 현실적인 범위로 조율하는 것이 만족도의 출발점입니다.
...
## 제가 세션 내내 레퍼런스를 껐다 켰다 하는 이유

저는 믹스 세션을 시작하면 작업 트랙 바로 옆에 레퍼런스 곡 두세 개를 항상 로드해 둡니다. 그리고 30분에서 한 시간마다 한 번씩 켜서 몇 초 들어봐요. 이유는 단순합니다. 오래 앉아 있으면 제 귀도 예외 없이 거짓말을 하기 시작하거든요. 아까는 밝다고 느꼈던 고역이 어느새 당연해지고, 부족한 베이스가 충분하게 들립니다. 레퍼런스는 그 순간마다 저를 원점으로 되돌려 놓는 기준점이에요.

15년쯤 작업하다 보니 장르별로 제 나름의 레퍼런스 목록이 쌓였습니다. 발라드용, K팝용, 어쿠스틱용을 따로 정리해 두고 새 곡을 받으면 편성과 분위기가 가장 가까운 걸 꺼내 씁니다. 여기서 중요한 건 목록을 자주 갱신하는 거예요. 몇 년 전 히트곡을 기준으로 삼으면 지금 스트리밍에서 나오는 소리와 미묘하게 어긋나거든요.

한 가지 덧붙이면, 레퍼런스는 나를 위축시키는 도구가 아니라 방향을 잡아주는 나침반이어야 합니다. 상업 음원과 똑같이 안 나온다고 좌절할 필요 없어요. 그 곡엔 수십 시간의 믹싱과 좋은 녹음 재료가 들어가 있으니까요. 그래서 저는 늘 좋은 결과물의 절반은 녹음 단계에서 이미 결정된다고 봅니다. 녹음부터 제대로 잡고 싶다면 [스튜디오 이용 안내](/pricing)를 먼저 확인해 보세요.
