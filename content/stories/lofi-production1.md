...
피치 워블(Pitch Wobble)은 테이프 드라이브 모터의 속도 불안정성에서 나옵니다. 정확히 일정한 속도로 테이프를 구동하지 못하면 재생 피치가 미세하게 오르내리는 "와우-플러터(Wow-Flutter)" 현상이 발생합니다. iZotope Vinyl의 Warp 기능이나 VinylDistortion 같은 플러그인이 이 효과를 재현합니다.
...
로파이 힙합의 리듬은 1950~60년대 재즈 드러머들이 연주하던 스윙 그루브에서 왔습니다. 정확히 정량화(Quantize)된 미디 드럼 대신, 사람이 직접 연주한 것처럼 박자가 약간 앞이나 뒤로 쏠리는 "루즈한" 타이밍이 로파이 힙합의 핵심 리듬감입니다. DAW에서 드럼을 프로그래밍할 때 퀀타이즈를 50~56% Swing으로 설정하거나, 8분음표 그리드에서 드럼 노트들을 10~20ms 범위 내에서 무작위로 이동시키는 방식이 사용됩니다.
...
스튜디오 놀에 들어오는 로파이·체일웨이브 보컬 녹음·믹싱 의뢰에서 자주 손이 가는 영역입니다.
...
## 로파이 편곡에서 제가 계속 붙잡는 감각

좋은 로파이와 어정쩡한 로파이를 가르는 건 플러그인 목록이 아니라 '얼마나 비워두느냐'라는 판단입니다. 위쪽 악기별 세팅이 '무엇을 거는가'였다면, 여기서는 '어디까지 손대지 않는가'를 이야기하고 싶습니다. 편곡 단계에서 제가 계속 되뇌는 세 가지입니다.

**트랙 수를 먼저 의심한다**

로파이가 안 풀릴 때 열이면 아홉은 요소가 너무 많습니다. 피아노·베이스·드럼·패드·샘플을 다 깔아놓고 "왜 답답하지" 하는 경우가 많은데, 저는 트랙 하나를 빼보라고 합니다. 빼도 곡이 성립하면 원래 없어도 되는 트랙이었던 겁니다. 로파이의 여백은 뮤트 버튼에서 나옵니다.

**틀린 걸 다 고치지 않는다**

퀀타이즈로 박자를 다 맞추고 피치를 다 잡으면 로파이 특유의 '사람이 친 느낌'이 사라집니다. 본문에서 스윙과 디튠을 이야기했지만, 실전 기준은 더 단순합니다. "고칠까 말까 망설여지는 어긋남은 그냥 둔다." 살짝 밀린 하이햇, 미묘하게 흐린 피치가 그 곡을 로파이로 만듭니다.

**분위기를 코드보다 먼저 정한다**

밤 카페인지, 비 오는 오후인지, 새벽 작업실인지 — 그 온도를 먼저 정하면 코드와 악기 선택이 저절로 좁혀집니다. 반대로 코드부터 예쁘게 짜놓고 분위기를 나중에 입히려 하면 이것저것 얹게 되고, 결국 첫 번째 문제인 트랙 과다로 돌아옵니다.
