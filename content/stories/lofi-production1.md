...
faq:
  ...
  - q: "로파이 힙합 비트는 어떻게 만드나요?"
    a: "슬로우 템포(70~90 BPM), 재즈 또는 소울 샘플을 기반으로 한 코드 진행, 드럼 루프에 비닐 시뮬레이터 적용, 피아노·기타·베이스를 살짝 디튠(Detune)해 빈티지 느낌을 냅니다. 멜로디는 심플하고 반복적인 것이 중요합니다."
  - q: "비닐 노이즈(레코드 크랙클) 효과는 어떻게 추가하나요?"
    a: "비닐 노이즈 샘플을 트랙에 추가하거나 RC-20 Retro Color, iZotope Vinyl, Waves Abbey Road Vinyl 같은 플러그인을 사용합니다. 볼륨은 -20~-15dB 수준으로 은은하게 깔아두면 됩니다."
...
로파이(Lo-Fi)는 고음질을 추구하는 현대 프로덕션과 반대로 빈티지한 질감, 노이즈, 불완전함을 의도적으로 가미해 따뜻하고 아날로그적인 사운드를 만드는 장르입니다.
...
1. 레코드 크랙클 (Vinyl Noise)
  - 비닐 레코드 재생 노이즈
  - -20~-15dB로 은은하게

2. 테이프 새츄레이션 (Tape Saturation)
  - 아날로그 테이프 녹음 특성
  - 따뜻한 배음, 부드러운 하이엔드

3. 피치 워블 (Pitch Wobble)
  - 테이프 드라이브 피치 불안정성 재현
  - Vinyl 플러그인의 Warp 기능

4. 로우-파이 필터링
  - **하이엔드 롤오프**: 8~10kHz 이상 감소
  - **미드 부스트**: 1~3kHz 약간 강조
  - 완성도 낮은 모니터 느낌
...
- **템포**: 70~90 BPM (슬로우)
- **드럼**: 스윙 퀀타이즈 (52~56% swing)
- **히햇**: 불규칙한 패턴, 사람이 친 느낌
- **스네어/림**: 타이트하고 작은 볼륨

### 코드 진행

- 재즈 코드 (maj7, min7, 9th, 13th)
- 2~4코드 반복 루프
- 피아노 또는 기타 보이싱
