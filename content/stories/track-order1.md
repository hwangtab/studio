...
summary: >-
  앨범·EP 트랙 순서 결정 과정을 작곡·편곡·녹음 흐름에 맞춰 정리합니다. 홈레코딩에서 적용할 방법과 Studio NOL 제작 상담 전 준비할 내용을 함께 확인하세요.
faq:
...
## Studio NOL 앨범 트랙 배열 의뢰에서 자주 손보는 3가지

스튜디오 놀에 들어오는 앨범 트랙 순서 배열 의뢰에서 자주 손이 가는 영역입니다.

**1. 1번 트랙 — 가장 강한 곡**

청자는 1번 트랙으로 앨범 인상을 결정합니다. 가장 강한 곡을 앞에 배치합니다.

**2. 키 흐름 — 5도권 인접 키**

트랙 간 키 변화는 5도권 인접 키로 자연스럽게 설계합니다.

**3. 외부 청취 테스트 — 3명 이상 비교**

배열 후 3명 이상에게 두 버전을 들려주고 어느 버전이 더 자연스러운지 확인합니다.
...
## 트랙 순서는 곡을 다 만든 뒤 시작되는 마지막 작곡이에요

앨범 배열을 의뢰하시는 분들께 저는 "이건 편집이 아니라 마지막 작곡"이라고 말씀드려요. 좋은 곡 열 개를 아무렇게나 늘어놓으면 열 개짜리 재생목록이지만, 순서를 설계하면 하나의 여정이 됩니다. 개별 곡으로 들을 때와 흐름 속에서 들을 때 인상이 완전히 달라지거든요.

가장 먼저 잡아야 할 건 1번 트랙이에요. 스트리밍 시대에는 앞 트랙 몇 곡의 스킵률이 앨범 전체 성과를 좌우하기 때문에, 가장 강하고 자기 색이 분명한 곡을 맨 앞에 세워야 합니다. 그다음은 에너지 곡선이에요. 처음부터 끝까지 세게만 가면 귀가 지치고, 계속 잔잔하면 늘어져요. 강-강-이완-재상승-여운처럼 오르내림을 설계하고, 밝은 곡 다음엔 발라드를 넣어 숨을 틔워 주세요. 마지막 곡은 가장 감정적이거나 사색적인 곡으로 여운을 남기고요.

트랙 사이 연결도 놓치기 쉬운 부분입니다. 인접한 두 곡의 키가 단3도 관계(C장조→A단조)면 감성이 자연스럽게 이어지고, 반음 차이면 긴장과 기대가 생겨요. 어색하면 인터루드를 끼우거나 무음 간격을 4~5초로 늘려 심리적 경계를 만드시면 됩니다. 다 짰으면 반드시 처음부터 끝까지 한 번에 재생해 검증하고, 두 버전을 서너 명에게 들려 보세요. 앨범을 세상에 내보내는 다음 단계는 [EP·싱글 발매 가이드](/stories/ep-release1)에서 다룹니다.
