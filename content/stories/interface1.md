...
## 인터페이스를 바꿔도 소리가 그대로인 이유

장비 상담을 하다 보면 '인터페이스를 더 좋은 걸로 바꿨는데 왜 소리가 안 달라지냐'는 질문을 정말 자주 받습니다. 냉정하게 말하면, 입문기부터 중급기 구간에서 인터페이스 등급 차이는 대부분의 사람이 블라인드로 구분하지 못합니다. 소리를 실제로 바꾸는 건 그 앞단, 즉 방의 울림과 게인을 어떻게 잡았느냐예요.

이 글에서 게인 스테이징을 –18~–12dBFS로 잡으라고 한 데는 분명한 이유가 있습니다. 저는 편집 단계에서 노이즈 플로어가 같이 올라와 히스가 낀 소스를 수도 없이 받아봤는데, 열에 아홉은 녹음 때 게인을 너무 낮게 잡아 두고 나중에 소프트웨어로 볼륨을 끌어올린 경우였어요. 인터페이스가 비싸든 싸든, 이 한 가지를 못 지키면 결과물은 똑같이 지저분해집니다.

버퍼 사이즈도 마찬가지입니다. 본문의 128~256샘플은 '녹음할 때' 기준이라는 걸 기억하세요. 녹음이 끝나고 플러그인을 잔뜩 걸어 믹스하는 단계에서 버퍼가 낮은 채로 두면 CPU가 튀고 소리가 끊깁니다. 녹음은 낮게, 믹스는 512샘플 이상으로 올려두는 걸 습관처럼 오가면 됩니다. 이건 장비가 아니라 순서의 문제예요.

그래서 입문 장비를 다 갖췄는데도 결과가 아쉽다면, 다음 투자는 더 비싼 인터페이스가 아니라 '한 번 제대로 된 환경에서 녹음해보는 경험'인 경우가 많습니다. 내 홈 셋업이 어디서 새는지는 통제된 방에서 같은 목소리를 담아보면 금방 드러나거든요. 필요하시면 [요금 안내](/pricing)를 참고해 반나절만 잡아 비교해보셔도 좋습니다.
