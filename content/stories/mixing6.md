...
## Studio NOL 믹싱 세션에서 미터를 보는 방식 — LUFS 기준과 레벨 체크 루틴

미터링을 잘못 이해하면 소리가 아닌 숫자를 믹싱하게 됩니다.

**케이스 1: LUFS만 보다가 다이나믹을 죽이는 실수**
스트리밍 기준인 -14 LUFS를 맞추려고 마스터 버스 리미터를 과도하게 걸면 후렴구가 터지는 느낌이 사라집니다. Studio NOL 믹싱 세션에서 믹스 버스 단계의 Short Term LUFS는 -10~-8 LUFS를 목표로 하고, 마스터링 단계에서 최종 Integrated를 -14 LUFS로 맞춥니다. 믹싱 단계에서 -14를 먼저 맞추려 하면 마스터링 여지가 없어집니다.

**케이스 2: 피크 미터만 보고 레벨을 판단하는 실수**
보컬 트랙에서 피크가 -6dBFS인데 실제 모니터에서 소리가 너무 작게 들리는 경우가 있습니다. 피크는 순간 최대치라 지속 에너지를 반영하지 못합니다. 이럴 때 VU미터나 RMS를 함께 보면 "피크 -6이지만 RMS -20" 같은 문제가 바로 드러납니다. 개별 트랙 레벨은 피크 + VU를 함께 확인하는 것이 기준입니다.

**케이스 3: 믹스 레벨 체크 루틴 — 귀 먼저, 숫자 나중**
Studio NOL에서 믹스를 시작할 때 미터는 먼저 끄고 귀로만 들으며 밸런스를 잡습니다. 어느 정도 귀로 만족스러운 밸런스가 나오면 그때 피크·LUFS를 확인합니다. 숫자가 먼저이면 "LUFS가 안 맞는데 왜 좋게 들리지?"라는 혼란이 오고, 귀가 먼저이면 숫자는 확인 도구가 됩니다. 미터링은 귀를 대체하는 것이 아니라 귀의 판단을 검증하는 도구입니다.
