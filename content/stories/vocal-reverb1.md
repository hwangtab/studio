...
## 보컬 리버브는 '들리게' 걸면 이미 과한 거예요

보컬에 리버브를 거는 일은 다른 악기와 결이 좀 달라요. 가사가 전달돼야 하니까요. 그래서 제가 세션에서 제일 먼저 만지는 건 리버브 종류나 디케이가 아니라 프리딜레이예요. 값을 20~40ms 정도 줘서 리버브가 보컬 바로 뒤가 아니라 한 박자 늦게 시작되게 하면, 청자가 자음과 딕션을 먼저 또렷이 듣고 나서 공간감을 느끼거든요. 프리딜레이가 0이면 아무리 좋은 리버브도 가수 입에 안개를 씌운 것처럼 뭉개져요.

거는 방식은 무조건 센드/리턴이에요. 보컬 채널에서 리버브 버스로 조금씩 보내고, 그 양으로 깊이를 조절하는 거죠. 이때 센드를 올리는 기준은 딱 하나예요. "리버브가 명확하게 들리기 시작하면 이미 지나친 것." 있는 듯 없는 듯, 리버브를 뮤트했을 때 비로소 "어, 허전하네" 싶은 정도가 리드 보컬엔 딱 맞아요. 대놓고 울리는 리버브는 발라드 후렴의 아주 짧은 순간이나 애드립에나 어울려요.

한 가지 더, 빠른 곡에서 긴 꼬리의 리버브를 욕심내면 음절끼리 겹쳐 지저분해져요. 130BPM이 넘어가면 디케이를 1초 안쪽으로 줄이거나, 아예 리버브 대신 짧은 스테레오 딜레이로 공간을 만드는 게 훨씬 깔끔해요. 딜레이와 리버브를 어떻게 나눠 쓰는지는 [보컬 딜레이 완전 가이드](/stories/vocal-delay1)에서 더 다뤄요.
