...
faq:
  - q: 피아노 4도 보이싱·쿼탈 하모니·McCoy Tyner을 처음 연습할 때 어디서부터 시작해야 하나요?
    a: >-
      ...
      쿼탈 보이싱 순서로 진행하면 좋습니다. 기초 단계를 익힌 뒤 다음 단계로 넘어가세요.
  - q: 피아노 4도 보이싱·쿼탈 하모니·McCoy Tyner 연습할 때 무엇을 신경 쓰면 좋나요?
    a: >-
...
## 콰탈 보이싱은 3도로 쌓던 화음을 4도로 갈아엎는 소리예요

우리가 아는 대부분의 코드는 3도씩 쌓아 올린 거예요. 그런데 이걸 4도 간격으로 쌓으면 완전히 다른, 탁 트이고 현대적인 소리가 나요. 이게 콰탈 보이싱이에요. McCoy Tyner나 Herbie Hancock의 그 시원하고 모던한 사운드가 바로 이 4도 적층에서 나와요. 3도 화음이 '이건 무슨 코드'라고 성격이 뚜렷한 데 비해, 4도로 쌓으면 근음이 뭔지 모호해지면서 넓게 열린 느낌이 들어요. 익숙한 코드를 4도로 다시 쌓아 그 소리 차이를 귀로 느끼는 것부터 시작하세요.

콰탈 보이싱이 가장 빛나는 자리는 모달 재즈예요. So What이나 Impressions처럼 한 코드 위에 오래 머무는 곡에서는, 3도 화음을 반복하면 금방 지겨워지는데 4도 보이싱은 그 모호하게 열린 색 덕분에 오래 머물러도 질리지 않아요. 그래서 모드 하나를 정해 그 위에서 4도 보이싱을 이리저리 옮겨 보는 연습이 좋아요.

연습은 양손을 분리해서 하세요. 왼손으로 4도 보이싱을 잡고 오른손으로 멜로디를 얹는 게 기본 구도인데, 왼손 보이싱이 자동화되지 않으면 오른손이 자유롭지 못해요. 왼손 콰탈 보이싱만 BPM 80쯤에서 생각 없이 나올 때까지 굳힌 다음 오른손을 더하세요. 이런 낯선 보이싱은 소리를 또렷이 들으며 반복해야 손에 붙어서 방음 공간이 유리해요. 스튜디오 놀 개인실에서 4도의 열린 소리를 실컷 울려 보세요. 이 보이싱이 활약하는 모달 재즈 전반은 [피아노 모달 재즈 연습](/stories/practice-room-piano-modal1)에서 이어집니다.
