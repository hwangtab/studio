...
summary: Mid/Side 처리·EQ·컴프 활용과 스테레오 폭 제어. Studio NOL 보컬 믹싱·마스터링 운용 기준.
faq:
...
## Studio NOL 보컬 믹싱·마스터링에서 Mid/Side 처리를 쓰는 3가지 케이스

스튜디오 놀 보컬 믹싱·마스터링에서 Mid/Side 처리가 결과를 바꾸는 반복 활용 케이스입니다.

**1. 보컬 리버브 — Side 100Hz 이하 컷**

리버브 적용 보컬의 Side(스테레오) 100Hz 이하를 Mid/Side EQ로 컷하면 저역이 좌우로 퍼지는 탁함이 사라집니다. 중심(Mid)의 보컬 본체는 그대로 두고 공간감만 깔끔하게 정리하는 핵심 기법입니다.

**2. 마스터링 — Side 부스트로 스테레오 폭 확장**

마스터 단계에서 5~10kHz Side를 살짝 부스트(+1~2dB)하면 자연스러운 스테레오 폭 확장이 가능합니다. 단, 모노 호환성을 잃지 않도록 120Hz 이하는 강제 모노로 유지합니다.

**3. 모노 호환성 점검 — Side만 모니터링**

마스터 버스에 Mid/Side 모니터링 토글을 두면 Side만 들어보며 모노 환경에서 사라질 요소를 미리 확인할 수 있습니다. Side 단독으로 들었을 때 보컬·킥·베이스가 들리면 모노에서 균형이 깨질 수 있다는 신호입니다.

---

## 마치며

MS 처리는 마스터링의 정밀도를 한 단계 높이는 고급 기법입니다. Mid와 Side를 분리하는 순간, 믹스의 모든 요소가 독립적으로 조정 가능한 정밀 기계가 됩니다. 처음에는 변화량을 작게 유지하고, 항상 모노 체크를 병행하는 습관을 들이면 MS 처리가 믹스의 약점을 강점으로 바꾸는 유용한 도구가 됩니다.

## 스테레오 폭을 넓힐 때 지키는 것

Mid-Side는 믹스를 넓고 선명하게 만드는 강력한 도구지만, 잘못 쓰면 저역이 무너지고 모노에서 소리가 사라져요. 세션에서 이걸 다룰 때 제가 지키는 원칙을 알려드릴게요.

**저역은 무조건 가운데(Mid)로 모읍니다.** 100Hz 이하를 좌우로 벌리면 위상이 틀어져 저역이 힘을 잃어요. 킥·베이스의 심지는 항상 모노로 단단히 두고, 넓히는 건 그 위 대역에서만 하는 게 원칙입니다.

**넓힐 건 보컬이 아니라 주변 악기입니다.** 리드 보컬은 가운데(Mid)에서 또렷해야 존재감이 살아요. 대신 기타·신스·코러스 같은 주변 악기를 사이드(Side)로 벌리면, 보컬은 중심을 지키면서 전체는 넓어집니다. 무엇을 세우고 무엇을 벌릴지를 먼저 정하세요.

**넓힌 뒤엔 반드시 모노로 확인합니다.** 스테레오에서 멋지게 벌어진 소리가 모노로 합치면 위상 상쇄로 사라질 수 있어요. 클럽·매장·폰 스피커는 사실상 모노라, 모노 호환성 체크는 선택이 아니라 필수입니다.
