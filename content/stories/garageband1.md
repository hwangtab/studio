...
녹음을 시작하기 전에 오디오 인터페이스와 프로젝트의 샘플레이트를 같은 값으로 맞춰두는 게 먼저입니다. 둘이 어긋나면 재생 속도나 피치가 미묘하게 틀어질 수 있거든요.
...
무료라고 하면 어딘가 부족할 거라 지레짐작하시는데, 저는 개러지밴드로 녹음한 파일을 실제 믹싱 소스로 여러 번 받아봤습니다. 마이크와 인터페이스, 그리고 녹음하는 방만 조금 잡아두면 여기서 나온 드라이 보컬은 유료 DAW로 녹음한 것과 구분하기 어려울 만큼 깨끗해요. 도구가 소리의 상한을 정하는 게 아니라, 소리를 담는 환경이 정합니다.

다만 개러지밴드를 '녹음 도구'로 쓰는 것과 '완성 도구'로 쓰는 건 구분하시면 좋겠습니다. 녹음과 간단한 정리까지는 충분하지만, 마지막 믹싱·마스터링 단계의 미세한 판단은 모니터 환경이 받쳐줘야 하거든요. 그래서 집에서는 개러지밴드로 좋은 테이크를 뽑는 데 집중하고, 마감은 제대로 된 방에서 하시길 권합니다.

파일을 넘기실 때 딱 하나만 지켜주시면 제 쪽 작업이 훨씬 수월해집니다. 본문에 적은 대로 무손실(AIFF/WAV)로 내보내고, 보컬과 MR을 각각 따로 주세요. MP3로 압축된 파일은 이미 정보가 깎여 나간 상태라 손볼 여지가 줄어듭니다.

집에서 개러지밴드로 녹음한 곡을 발매까지 끌고 가고 싶으시면, 믹싱·마스터링부터 유통까지 한 번에 정리하는 [음원 발매 프로젝트](/release-project)로 이어가는 방법도 있습니다. 어디까지 집에서 하고 어디부터 맡길지, 그 경계를 함께 잡아드리는 게 제 몫이에요.
