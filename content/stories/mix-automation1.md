...
summary: 볼륨 오토메이션·게인 라이딩·팬·플러그인 파라미터 오토메이션 실전. Studio NOL 보컬 믹싱 운용 기준.
faq:
  - q: 믹싱 오토메이션이란 무엇인가요?
    a: >-
...
## Studio NOL 보컬 믹싱에서 오토메이션을 가장 자주 쓰는 3가지 케이스

스튜디오 놀 보컬 믹싱 세션에서 오토메이션이 결과를 바꾸는 반복 케이스입니다.

**1. 보컬 게인 라이딩 — 컴프 전에 다이나믹 정리**

다이나믹 폭이 큰 보컬은 컴프 적용 전에 클립 게인 또는 볼륨 오토메이션으로 각 구절의 레벨을 -3~5dB 단위로 미세하게 조정합니다. 컴프에 의존하는 것보다 자연스러운 다이나믹 정리가 가능해 컴프 GR을 -3~5dB로 가볍게 유지할 수 있습니다.

**2. 코러스 진입 — 보컬·리버브 동시 부스트**

코러스 진입 직전에 보컬 채널을 +1~2dB, 리버브 센드를 +2~3dB 동시에 살짝 부스트하면 청자가 무의식적으로 "코러스가 시작됐구나"를 느낍니다. 이 미세한 오토메이션이 곡 전체의 임팩트를 결정합니다.

**3. 후렴 마지막 음절 — 페이드 또는 디크레셴도**

후렴 끝 음절이 어색하게 끊기지 않도록 볼륨 오토메이션으로 자연스러운 페이드를 그립니다. Release를 잘못 설정한 컴프로 처리하는 것보다 수동 오토메이션이 훨씬 자연스러운 결과를 만듭니다.
