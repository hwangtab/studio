...
|--------|-----------|---------------|
| DistroKid | 연 $24.99 구독료만(Musician) | 100% |
...
본 가이드의 스트림당 단가·시나리오 시뮬레이션·차감 순서는 국내에서 통용되는 일반적인 수준을 참고해 정리한 수치이며, 특정 정산 내역을 근거로 산출한 값이 아닙니다. 실제 단가는 플랫폼·국가·구독 형태(무료/유료)·계약 유형에 따라 큰 폭으로 변동하며, KOMCA 분배율과 문화체육관광부 분배 기준 역시 개정 시점에 따라 달라질 수 있으므로 정산 시점에 각 기관 공식 자료로 재확인하시기 바랍니다.
...
멜론 기준으로만 계산하면 단가 5~7원 × 1억 = 5억~7억원이지만, 플랫폼 수수료·KOMCA·유통사 차감 후 아티스트 수령액은 약 3억~5억원 수준입니다. 스포티파이 1억 스트리밍은 단가($0.003~$0.005)가 낮지만, 환율 1,400원 기준으로 환산하면 약 4.2억~7억원입니다. 즉 두 플랫폼의 수령액 규모는 비슷한 수준입니다.
