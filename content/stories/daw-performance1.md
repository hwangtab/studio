...
faq:
  ...
  - q: "버퍼 사이즈(Buffer Size)는 어떻게 설정하나요?"
    a: "녹음 시: 64~128 샘플 (레이턴시 최소화). 믹싱 시: 512~1024 샘플 (CPU 부하 감소). 마스터링 시: 1024~2048 샘플. 녹음 중 팝·클릭 노이즈가 들리면 버퍼를 올리고, 모니터링 레이턴시가 신경 쓰이면 버퍼를 낮추세요."
  - q: "트랙 프리징(Freeze)이란 무엇인가요?"
    a: "플러그인이 많이 걸린 트랙을 일시적으로 렌더링(오디오 변환)해 CPU 부담을 줄이는 기능입니다. 프리즈 상태에서는 플러그인 파라미터를 변경할 수 없지만, 언프리즈하면 다시 원본 상태로 돌아옵니다. Ableton·Logic·Pro Tools 모두 지원합니다."
