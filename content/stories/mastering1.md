...
마스터링이란 믹싱이 끝난 스테레오 파일을 스트리밍·CD·유튜브 같은 발매 환경에 맞게 마지막으로 다듬는 단계입니다. 전체 EQ, 음압(LUFS), 피크, 스테레오 폭, 곡 간 톤 일관성을 확인해 “어디서 들어도 어색하지 않은 최종본”으로 만듭니다.
...
같은 엔지니어가 믹싱과 마스터링을 모두 맡더라도, Studio NOL에서는 두 단계 사이에 반드시 시간을 둡니다.
...
"마스터링 비용이 얼마예요?"는 자주 받는 질문이에요. 결론부터 말씀드리면 싱글 1곡 기준 5~15만원 선이고, 작업 범위·엔지니어 경력·옵션에 따라 달라져요. 아래 표로 시세와 기간을 정리했어요.
