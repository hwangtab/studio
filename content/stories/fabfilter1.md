...
## Pro-Q3를 실제 세션에서 이렇게 씁니다

FabFilter는 화면이 예뻐서 다들 EQ로만 쓰는데, 사실 진짜 힘은 다른 데 있어요. 제가 세션에서 이 도구들을 실제로 어떻게 쓰는지 알려드릴게요.

**Pro-Q3의 진짜 무기는 Dynamic EQ입니다.** 보컬의 특정 구간에서만 치찰음이나 부밍이 튈 때, 그 대역을 항상 깎으면 다른 구간이 얇아져요. Dynamic 모드로 걸어두면 문제가 생기는 순간에만 그 주파수를 눌러주기 때문에, 소리를 해치지 않고 거슬리는 부분만 잡을 수 있습니다. 정적 EQ보다 훨씬 자연스러워요.

**디에싱·펌핑은 Pro-MB(멀티밴드)로.** 시빌런스(S 발음)나 저역의 펌핑처럼 특정 대역만 압축하고 싶을 때 멀티밴드 컴프가 정밀합니다. 대역을 좁게 잡아 그 안에서만 작동시키면, 전체 톤은 그대로 두고 문제 대역만 다스릴 수 있어요.

**마스터 마무리는 Pro-L 2로 True Peak를 잡습니다.** 마지막 리미팅 단계에서 True Peak 실링을 -1dBTP로 잡아두면 손실 압축 과정의 인터샘플 피크 왜곡을 막을 수 있어요. 음압을 무리하게 올리기보다 이 안전선을 지키는 게 우선입니다.
