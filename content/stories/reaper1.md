...
## 리퍼는 '내 손에 맞게 깎아내는' 재미로 쓰는 도구예요

리퍼로 상담 오시는 분께 제가 가장 먼저 하는 말은, 리퍼를 다른 DAW처럼 쓰지 말라는 거예요. 리퍼의 진짜 힘은 가격($60 영구 라이선스)이 아니라 '내 작업 방식에 맞게 깎아낼 수 있다'는 데 있거든요. 처음엔 텅 빈 회색 화면이 낯설지만, 자주 하는 동작 서너 개를 커스텀 액션으로 묶고 단축키 한 개에 걸어두면, 무거운 상용 DAW보다 훨씬 빠르게 녹음-컴핑-렌더가 돌아가요. 저는 보컬 녹음용으로 트랙 색·미터·페이더 크기까지 다 바꾼 전용 레이아웃을 하나 만들어 두고 씁니다.

시작하자마자 꼭 하실 건 두 가지예요. 하나는 SWS Extension 설치(무료). 이걸 깔아야 컴핑 모드, 리전 반복 녹음 같은 프로 워크플로가 열려요. 다른 하나는 액션 리스트(Actions → Show action list)를 한 번 훑어보는 거예요. 리퍼는 웬만한 기능이 다 액션으로 노출돼 있어서, 여기서 원하는 동작을 찾아 단축키를 직접 붙이는 게 리퍼를 '내 것'으로 만드는 핵심입니다. 내장 ReaEQ·ReaComp도 화려하진 않지만 보컬 처리엔 부족함이 없어요.

리퍼는 가볍고 자유로운 대신 스스로 세팅을 잡아야 하는 도구라, 녹음 자체의 기본기가 받쳐줘야 진가가 나와요. 셀프 녹음 환경을 정돈하는 요령은 [보컬 셀프 녹음 방법 가이드](/stories/selfrecord1)에 정리해 뒀으니 리퍼 세팅과 함께 맞춰 보세요.
