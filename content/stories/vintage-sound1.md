...
① Neve 1073 EQ 에뮬레이션
   → 100Hz 약한 부스트 (+1~2dB)
   → 12kHz 공기감 부스트 (+2dB)
② 진공관 컴프레서 (LA-2A 에뮬레이션)
   → 느린 어택, 자동 릴리즈
③ 테이프 새추레이션 (J37 또는 Kramer)
   → Drive: 30~40% (가볍게)
   → Bias: +1~2 (따뜻함 강조)

[빈티지 드럼 사운드]
① 70년대 드럼 샘플 또는 스테레오 룸 마이크
② 버스에 테이프 새추레이션
③ 빠른 어택 트랜지언트 쉐이퍼로 스냅 추가
④ 고역 6~8kHz 약간 컷 (비닐 느낌)

[로파이 힙합 사운드]
① iZotope Vinyl 또는 Baby Audio Lofi Juice
   → Noise: 20~40% (비닐 노이즈)
   → Wear: 15~30% (테이프 열화)
② 드럼: 피치다운 -3~-5 세미톤
③ EQ: 3kHz 이상 롤오프 (-6dB/oct)
④ Wow/Flutter: 1~5% (테이프 워블)
```
