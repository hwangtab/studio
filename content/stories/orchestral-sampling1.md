...
summary: 오케스트라 샘플 라이브러리·아티큘레이션·CC1·CC11 운용. Studio NOL 오케스트라 보컬 곡 믹싱 기준.
faq:
  - q: 오케스트라 샘플 라이브러리를 시작하려면 무엇부터 해야 하나요?
    a: >-
      먼저 목적을 결정하세요. 영화·TV 음악이라면 Spitfire Audio LABS (무료)부터 시작해 CSS(Cinematic
      Studio Strings) 또는 Spitfire Chamber Strings로 확장합니다. 일반 팝·발라드 오케스트레이션은
      Native Instruments SESSION STRINGS PRO 2가 접근하기 쉽습니다.
  ...
  - q: 오케스트라 샘플이 자연스럽지 않게 들리는 이유는 무엇인가요?
    a: >-
      ...
      체인지)로 자동화하고, 아티큘레이션(Legato·Staccato·Marcato)을 곡에 맞게 전환하면 크게 개선됩니다.
  - q: 오케스트라 팬닝(Panning)은 어떻게 설정하나요?
    a: >-
      실제 오케스트라 좌석 배치를 참고합니다. 제1바이올린(왼쪽), 제2바이올린(중앙왼쪽), 비올라(중앙), 첼로(중앙오른쪽),
      더블베이스(오른쪽), 목관(중앙 뒤쪽), 브라스(중앙~오른쪽 뒤), 타악기(맨 뒤)가 표준 배치입니다.
inlineFallback:
...
## Studio NOL 오케스트라 편곡 보컬 곡 의뢰에서 자주 손보는 3가지

스튜디오 놀에 들어오는 오케스트라 편곡 보컬 곡(시네마틱 발라드·뮤지컬·OST) 의뢰에서 반복적으로 손이 가는 영역입니다.

**1. 현악 + 보컬 — 1~3kHz 자리 정리**

오케스트라 현악(특히 바이올린)은 1~3kHz 보컬 명료도 대역에 강한 에너지가 있어 보컬과 충돌합니다. 보컬 휴지 구간에 현악 라인을 집중시키거나, 동시 진행 시 현악 음역을 보컬보다 한 옥타브 위·아래로 분리합니다.

**2. 다이내믹 컨트라스트 — 버스·코러스 오케스트라 밀도**

버스(인트로·1절)는 작은 편성, 코러스에서 풀 오케스트라로 폭발하는 편곡이 시네마틱 보컬 곡의 표준입니다. 모든 구간 오케스트라 밀도가 동일하면 보컬도 평탄해집니다.

**3. CC1·CC11 자동화 — 자연스러운 다이내믹**

오케스트라 라이브러리(Spitfire·Cinematic Studio Strings)는 CC1(Modulation Wheel)·CC11(Expression)로 자연스러운 다이내믹 변화를 만듭니다. 100% 양자화된 미디는 인공적으로 들리므로, 곡 흐름에 맞춘 CC 자동화가 필수입니다.
