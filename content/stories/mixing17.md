...
author: "스튜디오 놀"
category: "강좌"
tags: ["믹싱", "강좌", "컴프레서", "다이내믹스", "리미터"]
---
![클래식 컴프레서의 노브들](/images/hardware1.jpg)

"컴프레서가 제일 어려워요. 도대체 무슨 소리가 변하는지 모르겠어요."
많은 분들이 컴프레서(Compressor) 앞에서 좌절합니다. EQ처럼 소리가 확확 바뀌는 것도 아니고, 잘못 걸면 소리가 답답해지기만 하니까요.

하지만 컴프레서의 원리는 아주 간단합니다. 어렵게 생각하지 마세요.
**"소리가 너무 커지면 자동으로 볼륨을 줄여주는 장치"**
이게 끝입니다. 정말이에요.

## 1. 컴프레서 4대장 용어 완전 정복

이 4가지 파라미터만 이해하면 컴프레서는 여러분의 장난감이 됩니다.

*   **Threshold (문지방)**: "여기 넘어가면 혼난다!"
...
*   **Ratio (압축 비율)**: "얼마나 세게 혼낼까?"
    ...
    *   **4:1**: 단호한 선생님 (베이스, 스네어)
    *   **10:1**: 엄격한 교관 (피크 제어, 리미팅)
*   **Attack (반응 속도)**: "얼마나 빨리 때릴까?"
    *   **빠르게(Fast)**: 소리가 튀어나오자마자 즉시 잡습니다. 소리가 둥글고 뒤로 들어가는 느낌이 듭니다.
    *   **느리게(Slow)**: 앞부분의 '탁!' 하는 타격감(Transient)은 봐주고, 그 뒷부분부터 잡습니다. 드럼의 펀치감을 살릴 때 필수입니다.
...
"저는 소리를 크고 시원하게 만들고 싶은데, 왜 줄이나요?"
이게 컴프레서의 핵심 역설입니다.

1.  큰 소리를 눌러서 **작게 만듭니다.**
2.  그 덕분에 전체적인 음량 편차가 줄어들어 소리가 **'고르게(Consistent)'** 됩니다.
3.  이제 전체 볼륨(Make-up Gain)을 **올립니다.**
4.  결과: 작았던 소리는 커지고, 컸던 소리는 정돈되면서, 소리가 전체적으로 **두껍고, 단단하고, 꽉 차게(Solid)** 변합니다.
...
*   **단단하게 잡고 싶다**: -5dB ~ -10dB까지 푹푹 찍히도록 과감하게 거세요.
*   귀로 듣는 훈련과 눈으로 보는 확인을 병행하면 금방 감을 잡을 수 있습니다.

## 4. 고급 기술: 패러렐 컴프레션 (Parallel Compression)

"컴프레서를 걸었더니 소리는 단단해졌는데, 생동감이 죽어서 답답해요."
그럴 땐 **'패러렐 컴프레션(뉴욕 스타일 컴프레션)'**이라는 비기를 쓰세요.

1.  드럼 트랙을 복사(또는 Send)해서 똑같은 트랙을 하나 더 만듭니다.
2.  복사된 트랙에 컴프레서를 **아주 과격하게** (Ratio 10:1 이상, GR -10dB 이상) 겁니다. 찌그러질 정도로요.
3.  이제 원래의 **깨끗한 트랙(Dry)**과 **과격하게 눌린 트랙(Wet)**을 적절히 섞어줍니다.

이렇게 하면 원음의 생생한 타격감(Transient)은 살아있으면서, 컴프레서가 주는 묵직한 바디감과 에너지를 동시에 얻을 수 있습니다. 두 마리 토끼를 다 잡는 셈이죠.

---

컴프레서는 귀로 듣는 것보다 **몸으로 느끼는 이펙터**에 가깝습니다.
소리의 '밀도'가 변하는 걸 느껴보세요. 헐렁하던 소리가 쫀쫀해지고, 산만하던 리듬이 그루브를 타는 그 마법 같은 순간을요.
