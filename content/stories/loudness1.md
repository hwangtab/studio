...
| 멜론·지니 | 확인 필요 | 확인 필요 | 정책 비공개 |
| 타이달 | -14 LUFS | -1 dBTP | 하이파이 스트리밍 |
| 사운드클라우드 | 확인 필요 | 확인 필요 | 정책 비공개 |
...
| 발라드 | -16 ~ -14 LUFS | 넓은 다이나믹 |
| 힙합·EDM | -9 ~ -13 LUFS | 높은 압축 |
| 재즈·어쿠스틱 | -16 ~ -18 LUFS | 가장 넓은 다이나믹 |
| 록·메탈 | -10 ~ -13 LUFS | 높은 압축 |

위 표는 스트리밍 정규화 이후에도 다이내믹을 최대한 지키고 싶을 때의 권장 범위입니다. 실제 상업 발매, 특히 K팝·힙합·EDM·록처럼 에너지가 핵심인 장르는 이보다 훨씬 높은 음압으로 마스터링되는 경우가 흔하며, 이는 아래 실전 지침과 장르별 확장 표에서 다시 다룹니다. 힙합과 EDM에서 -9 LUFS까지 높은 음압을 사용하는 이유는 장르적 관행과 클럽·공연 환경에서의 임팩트 때문입니다. 이 장르에서는 스트리밍 노멀라이제이션으로 볼륨이 낮춰지더라도 압축된 에너지와 밀도가 음악의 특성으로 인식됩니다. 반면 재즈와 클래식에서 -18 LUFS 수준의 낮은 음압을 사용하는 것은 넓은 다이나믹 레인지가 장르의 예술적 가치이기 때문입니다.
...
|--------|-----------|---------------|------|
| 스포티파이 (Normal, 기본값) | -14 LUFS | -1 dBTP | 기본값 |
| 스포티파이 (Quiet) | -19 LUFS | -1 dBTP | 사용자 설정 |
| 애플뮤직 | -16 LUFS | -1 dBTP | Sound Check |
| 유튜브뮤직 | -14 LUFS | -1 dBTP | 영상·음악 동일 |
| 아마존뮤직 | -14 LUFS | -2 dBTP | True Peak 더 엄격 |
| 타이달 | -14 LUFS | -1 dBTP | MQA 별도 |
| 멜론·지니·바이브 | 확인 필요 | 확인 필요 | 정책 비공개 |
| 사운드클라우드 | 확인 필요 | 확인 필요 | 정책 비공개 |
| 틱톡·인스타그램 | -14 LUFS | -1 dBTP | 숏폼 기준 |
| 넷플릭스 OTT | -27 LUFS | -2 dBTP | Netflix 자체 딜리버리 스펙 |

**스포티파이**는 전 세계 시장 점유율 1위 플랫폼이에요. 리스너가 선택하는 음량 모드는 세 단계인데, 기본값인 Normal 모드가 -14 LUFS, 더 크게 듣는 Loud 모드가 -11 LUFS, 더 작게 듣는 Quiet 모드가 -19 LUFS예요. True Peak은 -1 dBTP 이하가 표준이에요. 스포티파이는 -14보다 조용한 음원은 기본적으로 Positive Gain을 적용하지 않고 그대로 재생해요(Normal 모드 기준). 즉 -16 LUFS 음원은 -16 그대로 재생된다는 뜻이에요. 대다수 사용자는 별도 설정을 바꾸지 않아 기본값인 Normal 모드로 듣기 때문에, 마스터링 타깃은 여전히 -14 LUFS로 잡는 게 맞아요.
...
**멜론·지니·바이브** 등 국내 플랫폼은 재생 시 라우드니스 정규화를 적용하는지 여부를 공식적으로 밝히지 않고 있어요. 유통사가 납품 스펙으로 -14 LUFS·True Peak -1dBTP를 권장하는 경우는 있지만, 이는 유통사의 '납품 가이드라인'이지 플랫폼이 실제 재생 시 그 값으로 맞춰준다는 근거는 아니에요. 국내 플랫폼용으로 음압을 얼마나 높여도 되는지는 공개된 근거가 없으니, 발매 전 유통사에 직접 문의해 확인하는 걸 추천해요. 스포티파이 등 해외 플랫폼은 -14 LUFS 기준으로 정규화되니, 국내·해외 동시 발매라면 이 기준을 우선 고려하는 게 안전해요.

**사운드클라우드**는 다른 스트리밍 플랫폼과 달리 정규화 정책을 공식적으로 밝히지 않았어요. 업로드한 마스터가 거의 그대로, 혹은 정규화가 있더라도 약하게만 적용된다는 게 업계의 대체적인 인상이지만 정확한 수치는 확인되지 않았으니, 발매 전 직접 들어보고 확인하는 걸 추천해요. 데모 공유·언더그라운드 장르에서 "크게 들리는" 마스터를 선호하는 문화가 여기서 나왔다는 이야기도 있고요.

**틱톡·인스타그램** 등 숏폼 플랫폼은 -14 LUFS로 정규화해요. 숏폼은 피드에서 빠르게 지나가기 때문에 첫 1~2초의 임팩트가 중요하고, 피크 한도 -1 dBTP를 지켜 재인코딩 시 왜곡을 막는 것도 중요해요.

**넷플릭스·디즈니+ 같은 OTT 방송**은 음악 스트리밍과 완전히 달라요. 방송 표준은 미국 ATSC A/85 기준 -24 LUFS, 유럽 EBU R128 기준 -23 LUFS인데, 넷플릭스는 이보다 더 엄격한 자체 딜리버리 스펙인 -27 LUFS를 요구해요(지역 구분 없이 전 세계 공통 기준). True Peak은 -2 dBTP예요. 음악이 아닌 대사 기반 콘텐츠의 다이내믹을 보존하기 위한 기준이에요. 영화 OST를 공급하거나 OTT 삽입곡을 만든다면 이 기준으로 별도 마스터를 준비해야 해요.
...
**그렇다면 -14 LUFS가 무조건 좋은가?** 여기서 또 다른 함정이 있어요. 모든 곡을 기계적으로 -14 LUFS에 맞추면 장르 특성이 지워져요. EDM·힙합 같은 장르는 높은 음압과 꽉 찬 에너지가 장르적 정체성이에요. 이 장르를 억지로 -14 LUFS로 내리면 에너지가 빠져서 오히려 "밋밋"하게 들려요. 반대로 클래식·재즈·어쿠스틱은 넓은 다이내믹이 예술적 가치여서, -14 LUFS로 과하게 압축하면 음악의 호흡이 사라져요. 참고로 아래 실전 지침 수치는 앞서 나온 다이내믹 보존형 권장 표와는 관점이 달라요 — 실제 상업 발매 시장에서 관찰되는, 다소 더 높은 음압 범위예요.
...
장르별 권장 LUFS를 더 자세히 풀어드릴게요. 앞서 다룬 기본 표가 5개 장르였다면, 여기서는 9개 장르로 확장해서 각각의 이유까지 설명해요. 이 값들은 2026년 기준 스트리밍 플랫폼과 상업 마스터링 관행을 종합한 권장 범위이며, 절대 수치가 아니라 가이드라인이에요. 특히 K-pop처럼 에너지가 핵심인 장르는 앞서 다룬 다이내믹 보존형 권장치(-13~-14 LUFS)보다 훨씬 크게, 실제로는 -9~-6 LUFS대에서 발매되는 사례가 흔하다는 조사도 있어요 — 아래 표의 -8~-6은 '이상적 권장치'가 아니라 '실제 상업 관행에 가까운 값'으로 이해해주세요.
...
**K-pop 댄스 (-8 ~ -6 LUFS)** — 아이돌 댄스 트랙은 강렬한 첫 인상이 핵심이에요. MV·라이브 무대·클럽 플레이 등 다양한 매체에서 임팩트가 필요해서, 스트리밍 정규화로 일부 손실되더라도 컴프레션된 밀도감이 장르 정체성이에요. 2024년 발매된 주요 K-pop·팝 히트곡을 표본 조사한 자료에 따르면 다수가 -9~-7 LUFS 부근에서 측정됐다는 보도도 있어요(스트리밍 권장치 -14보다 훨씬 큰 수준이죠). 트와이스·에스파·뉴진스 등의 댄스 트랙이 이 범위에서 마스터링되는 경우가 많아요.
...
**2010년대 — 스트리밍 정규화 도입** — 2014년 스포티파이가 -14 LUFS 라우드니스 정규화를 공식 시작하면서 게임의 규칙이 바뀌어요. 애플뮤직도 Sound Check 기능으로 자체 정규화를 적용해왔는데, 정확한 도입 시점은 자료마다 달라 특정하기 어려워요. 유튜브·타이달·아마존뮤직 등 주요 플랫폼이 2015~2017년 사이에 순차적으로 정규화를 도입했고, 2018년경에는 전 세계 음악 소비의 70% 이상이 정규화 적용 환경으로 넘어갔어요. 이 시점부터 "음압을 높여도 오히려 음량이 줄고 다이내믹만 잃는다"는 공식이 확립됐고, 메이저 스튜디오부터 마스터링 철학이 바뀌기 시작했어요.
...
**한국 K-pop의 라우드니스 트렌드** — 한국 K-pop 씬은 다소 독특한 경로를 걸었어요. 2010년대 중반까지 한국 마스터링은 -7 ~ -9 LUFS 수준의 높은 음압이 관행이었어요. 음악방송·유튜브 뮤직비디오 위주 소비 환경에서 "첫 임팩트"가 중요했거든요. 2020년 이후 스포티파이·애플뮤직의 글로벌 진출이 본격화되면서도, K-pop 댄스 트랙은 여전히 -9 ~ -6 LUFS대의 높은 음압을 유지하는 경우가 많아요 — 스트리밍 정규화가 있어도 강렬한 임팩트를 우선하는 장르 관행은 쉽게 바뀌지 않은 셈이죠. 반면 발라드·OST는 -16 ~ -14 LUFS대의 넓은 다이내믹을 지키는 추세예요.
...
**1. 발라드·어쿠스틱 — -16 ~ -14 LUFS 범위 유지**

다이나믹 레인지가 풍부해야 하는 발라드·어쿠스틱 곡은 Apple Music -16 LUFS·Spotify -14 LUFS 사이인 -16 ~ -14 LUFS Integrated 범위를 유지합니다. 무리하게 -10 LUFS까지 끌어올리면 보컬 후렴의 폭발감이 사라져 음악이 평탄해집니다.

**2. EDM·힙합 — -9 ~ -6 LUFS, True Peak -1dBTP 절대 사수**

스트리밍 플랫폼이 -14 LUFS로 정규화하더라도 EDM·힙합 곡은 -9 ~ -6 LUFS에서 마스터링하는 것이 일반적입니다. 다만 True Peak가 -1dBTP를 초과하면 정규화 후 클리핑이 발생하므로 리미터 천장은 -1dBTP를 절대 넘지 않게 설정합니다.

**3. 유튜브·SoundCloud 동시 발매 — -13 LUFS 절충**

유튜브는 -14 LUFS 기준이 명확하지만, SoundCloud는 정규화 정책이 공식적으로 확인되지 않아 업로드한 마스터가 거의 그대로 재생되는 경우도 있습니다. 두 플랫폼에 동시 발매할 때는 -13 LUFS 전후로 절충하면 유튜브 정규화 후에도 자연스럽고 SoundCloud에서도 과도하게 크거나 작게 들리지 않는 안전한 지점이 됩니다. 플랫폼별로 별도 마스터를 만드는 것이 이상적이지만, 단일 마스터로 가야 할 때 -13 LUFS가 실용적인 절충점입니다.
...
## 마스터링하며 발매 직전에 늘 확인하는 것

발매 직전에 "음압이 애매한데 괜찮을까요?" 묻는 분이 정말 많아요. 마스터링을 하며 제가 마지막에 반드시 점검하는 세 가지를 알려드릴게요. 이것만 맞으면 대부분의 플랫폼에서 안전합니다.

**음압을 무작정 올리지 않습니다.** 스트리밍 플랫폼은 어차피 -14 LUFS 기준으로 음량을 정규화해요. 그보다 크게 밀어붙이면 플랫폼이 도로 낮추면서 다이내믹만 뭉개집니다. 크게 만드는 것보다 -14 LUFS 근처에서 '단단하게' 들리게 하는 게 목표예요.

**True Peak는 -1dBTP로 여유를 둡니다.** 화면상 0dB를 안 넘겼어도, 손실 압축(MP3·AAC) 과정에서 인터샘플 피크가 튀어 왜곡이 생길 수 있거든요. 마스터 리미터의 True Peak 실링을 -1dBTP로 잡아두는 습관이 이 사고를 막습니다.

**숫자보다 귀로 A/B 합니다.** 미터가 맞아도 안심하지 않아요. 내가 목표로 하는 상업 음원을 같은 음량으로 맞춰 나란히 들어보면, 내 곡이 얇은지 답답한지가 그제야 들립니다. 결국 마지막 판단은 미터가 아니라 귀입니다. 혼자 판단하기 부담스러우면 [발매 마스터링](/mixing-mastering)을 의뢰하는 방법도 있어요. 개념이 더 궁금하면 [마스터링 완전 가이드](/stories/mastering1)로 이어보세요.
