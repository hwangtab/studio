---
title: 드라마·영화 OST 녹음 가이드 — OST 보컬 준비부터 납품까지
date: 2026-04-06
author: 스튜디오 놀
category: 녹음 가이드
tags:
  - 드라마 OST 녹음
  - 영화 OST 보컬
  - OST 제작
  - 드라마 삽입곡 녹음
  - OST 보컬 준비
  - 영상 음악 녹음
  - OST 믹싱
thumbnail: /images/recording17.webp
summary: >-
  드라마·영화 OST 보컬 녹음의 전체 과정 — 감독 브리핑, 디렉션, 음악적 요구사항, 파일 납품 규격까지 OST 녹음을 처음 경험하는 보컬리…
faq:
  - q: OST 녹음은 일반 보컬 녹음과 어떻게 다른가요?
    a: >-
      OST 녹음은 영상의 장면·분위기에 맞는 감정 표현이 핵심입니다. 음악 감독의 세부 디렉션(장면 설명, 감정 키워드)을 따르며 반복
      수정이 일반적입니다. 파일 납품 규격(WAV 24bit 48kHz 등)이 방송·영화사별로 정해져 있습니다.
  - q: OST 보컬 섭외는 어떻게 이루어지나요?
    a: >-
      주로 음악 감독이 기존 데모 음원이나 SNS 포트폴리오를 보고 보컬을 섭외합니다. 보컬 에이전시 등록이나 SNS를 통한 활동이 섭외
      기회를 높입니다. 전문 스튜디오 녹음 경력과 고음질 데모가 어필 포인트가 됩니다.
  - q: OST 녹음 파일 납품 규격은 어떻게 되나요?
    a: >-
      방송사마다 다르지만 일반적으로 WAV 24bit 48kHz(방송 표준)를 요구합니다. 영화는 WAV 24bit 48kHz 또는
      24bit 96kHz를 요구하기도 합니다. 믹싱 납품 시 -1dBFS 이하 True Peak 기준을 지켜야 합니다.
  - q: 아마추어도 OST 녹음에 참여할 수 있나요?
    a: >-
      인디 영화, 단편 영화, 유튜브 드라마 등 소규모 OST에는 신인 보컬도 참여 기회가 있습니다. 우선 고품질 보컬 포트폴리오를 만들고
      SNS로 공개하는 것이 기회를 만드는 첫걸음입니다.
---
![드라마·영화 OST 녹음 가이드 — 스튜디오 놀](/images/recording17.webp)

## OST 보컬이란?

드라마·영화·애니메이션의 삽입곡 또는 주제곡을 부르는 보컬입니다. 영상의 분위기에 맞는 감정 표현이 핵심이며, 음악 감독의 디렉션 하에 여러 테이크를 녹음하는 것이 일반적입니다.

한국 드라마 OST 시장은 1990년대 멜로 드라마 붐과 함께 본격적으로 형성됐습니다. 1995년 드라마 '모래시계'의 테마 음악이 방영과 함께 차트를 휩쓸면서 드라마 OST가 독립적인 음반 시장으로 자리 잡았고, 1997년 '별은 내 가슴에' OST(신화)가 수십만 장 판매되면서 OST 발매가 공식 수익 구조로 정착했습니다. 2000년대 '겨울연가'(2002)와 '대장금'(2003) OST는 일본·중화권으로 수출되며 K-드라마 OST의 글로벌 시장성을 최초로 입증했습니다. 2010년대 '태양의 후예'(2016, 케이윌·에릭남·에이오에이 등 참여)와 '도깨비'(2017, 처음엔 미었는데·Stay With Me)는 OST 한 곡이 멜론 실시간 1위를 장기간 점령하며 드라마 OST가 메인스트림 음악 산업의 핵심 콘텐츠로 진입했습니다. 현재는 넷플릭스·웨이브·디즈니+ 등 OTT 오리지널 드라마의 OST 시장도 급성장 중이며, OTT 납품 규격(WAV 24bit 48kHz, -1dBFS True Peak)이 방송사와 동일하게 적용됩니다.

---

## OST 녹음 과정

[1단계] 곡 수령 및 브리핑

- 음악 감독에게 곡 파일·가사·장면 설명 수령
- 분위기·감정 키워드 파악
- 영상 클립 사전 시청 (가능한 경우)

[2단계] 사전 준비

- 곡 완벽 암기 (가사·멜로디)
- 키 조정 필요 시 음악 감독과 협의
- 보컬 워밍업 후 세션 시작

[3단계] 녹음 세션

- 가이드 트랙(반주) 들으며 보컬 녹음
- 음악 감독 디렉션에 따라 다양한 감정·표현 시도
- 여러 테이크 녹음 후 선택

[4단계] 파일 납품

- 선택된 테이크 WAV 파일 납품
- 규격: 방송 표준 WAV 24bit 48kHz
- 드라이 보컬 + 완성 믹스 모두 납품하는 경우도 있음

---

## OST 디렉션 받는 방법

| 디렉션 유형 | 설명 | 대응 방법 |
|-----------|------|---------|
| 감정 키워드 | "슬프게", "희망차게", "절제해서" | 키워드를 음색·다이나믹으로 표현 |
| 장면 설명 | "주인공이 떠나는 장면" | 장면을 상상하며 감정 입히기 |
| 레퍼런스 | "OOO 노래처럼" | 레퍼런스 곡 사전 청취 |
| 수정 요청 | "좀 더 담담하게" | 반복 녹음에 유연하게 대응 |

---

## OST 파일 납품 규격

### 방송 표준

- **포맷**: WAV
- **비트뎁스**: 24bit
- **샘플레이트**: 48kHz
- **True Peak**: -1dBFS 이하
- **채널**: 스테레오 (믹스) / 모노 (드라이 보컬)

### 영화 납품

- **샘플레이트**: 48kHz 또는 96kHz (감독 지시 따름)
- **비트뎁스**: 24bit

스테레오 믹스와 드라이 보컬을 따로 납품해달라는 요청이 많습니다.

### 온라인/유튜브

WAV 44.1kHz 24bit 또는 48kHz 24bit

- **LUFS**: -14 LUFS (유튜브 표준)

---

## OST 보컬 포트폴리오 만들기

### 필수 요소

- 고음질 보컬 데모 (전문 스튜디오 녹음)
- 장르별 다양한 감정 표현 샘플
  예: 발라드·서정·에너지·슬픔

- SNS 공개 또는 사운드클라우드 업로드

### 섭외 기회 확대

- 인디 영화 OST 공모전 참여
- 유튜브 드라마 OST 오디션 지원
- 음악 감독·프로듀서와 네트워크 형성

---

## 마치며

드라마·영화 OST는 보컬리스트로서 커리어를 높일 수 있는 귀한 기회입니다.

OST 녹음에서 가장 중요한 준비는 영상을 먼저 보는 것입니다. 가능하다면 해당 장면 클립을 세션 전에 반복 시청하면, 음악 감독의 디렉션이 시작됐을 때 "슬프게"나 "절제해서" 같은 감정 키워드가 즉시 구체적인 보컬 표현으로 연결됩니다. 가사의 의미와 영상 속 캐릭터의 상황을 연결한 상태에서 마이크 앞에 서면 같은 테이크도 설득력이 달라집니다. 음악 감독이 "같은 노래인데 느낌이 다르다"는 반응을 보일 때 대부분 이 맥락의 차이에서 옵니다.

OST 납품에서 드라이 보컬(리버브·딜레이 없는 원본 파일)을 별도로 보존하는 것은 필수입니다. 음악 감독이 드라이 보컬을 요청하는 경우가 많고, 이후 드라마 OST 앨범 발매 시 리믹스·리마스터를 위해 원본이 필요합니다. 방송 표준(WAV 24bit 48kHz)과 스트리밍 발매용(-14 LUFS 마스터)은 서로 다른 파일이므로, 드라이 보컬 파일·방송 납품 파일·스트리밍 마스터 파일을 모두 분리해 보관하는 것이 이후 재작업 요청에 대응하는 가장 안전한 방법입니다.

## OST 세션은 노래보다 장면을 먼저 부릅니다

OST 녹음을 여러 번 받아보면서 확실해진 게 하나 있습니다. OST에서 잘 부른다는 건 음정이 완벽한 것과는 조금 다른 문제라는 겁니다. 음악 감독이 원하는 건 '이 장면에 붙었을 때 울리는 소리'지 교과서 같은 발성이 아니거든요. 그래서 저는 의뢰가 들어오면 부르는 분께 곡보다 장면 클립을 먼저 달라고 합니다. 주인공이 어떤 상황에서 이 노래가 흐르는지 알고 마이크 앞에 서는 것과, 그냥 좋은 멜로디라 여기고 부르는 것은 같은 테이크여도 설득력이 완전히 다릅니다.

세션 진행 방식도 일반 녹음과 다릅니다. 감독의 디렉션이 "좀 더 담담하게", "여기선 참는 느낌으로"처럼 감정 언어로 오기 때문에, 저는 그걸 호흡량이나 마이크 거리 같은 구체적인 조작으로 번역해서 부르는 분께 전달합니다. 감정을 감정으로만 주문하면 부르는 사람이 막막해지지만, "이 소절은 숨을 반쯤 남기고 속삭이듯" 정도로 바꿔주면 바로 소리가 달라집니다.

납품 단계에서 제가 늘 강조하는 건 원본을 흩뜨리지 말라는 겁니다. 방송용 파일과 스트리밍용 마스터, 그리고 이펙트를 걸지 않은 드라이 보컬은 나중에 리믹스나 앨범 발매 요청이 들어왔을 때 전부 필요해집니다. 이 파일들을 세션 당일에 분리해서 정리해두는 것까지가 제가 생각하는 OST [녹음 세션](/pricing)의 마무리입니다. 몇 달 뒤 재작업 요청이 와도 당황할 일이 없어집니다.

---

[보컬 포트폴리오 만들기](/stories/portfolio1) | [오디션 준비 완전 가이드](/stories/audition1) | [음반 제작 가이드](/stories/album1)
