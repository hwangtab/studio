...
## 반주는 보컬의 자리를 비워둘 때 완성됩니다

MIDI로 혼자 편곡하다 보면 악기를 자꾸 더 얹고 싶어집니다. 가상악기가 무한하니 스트링도 넣고 신스도 쌓다 보면 어느새 반주가 꽉 차서, 정작 주인공인 보컬이 앉을 자리가 없어져요. 저는 보컬 곡 반주를 만들 때 '무엇을 넣을까'보다 '무엇을 비울까'를 먼저 생각하는데, 위 글에서 코드 파트에 공간을 남기라고 한 것과 같은 맥락입니다. 모든 박자를 채우지 않아야 노래가 숨을 쉽니다.

특히 멜로디와 반주가 같은 음역에서 부딪히는 경우가 흔합니다. 카운터멜로디나 스트링 라인을 보컬 멜로디와 같은 대역에 두면 서로 존재감을 갉아먹어서, 저는 보컬이 쉬는 프레이즈 사이 공간에 반주 라인을 배치하고 후렴에서만 레이어를 두껍게 올립니다. 절에서는 비우고 후렴에서 채우는 이 대비가 곡의 에너지 곡선을 만들어요.

그리고 MIDI 편곡의 가장 큰 장점은 보컬리스트 키에 맞춰 클릭 한 번으로 조를 바꿀 수 있다는 점입니다. 저는 MR을 만들 때 보컬리스트의 키를 먼저 확인하고 BPM을 협의한 뒤 코드→드럼·베이스→멜로디 순으로 쌓아요. 완성된 MR은 WAV와 함께 원본 MIDI 파일도 챙겨두시면 세션 현장에서 키·BPM을 바로 손볼 수 있습니다. 편곡을 직접 배워 곡 완성도까지 끌어올리고 싶으시면 [프로듀싱 레슨](/lesson)에서 진행 중인 곡을 놓고 함께 다듬어 드립니다.
