...
summary: >-
  딜레이 믹싱 설정 기준과 작업 순서를 정리합니다. 홈레코딩에서 바로 적용할 체크포인트와 Studio NOL 믹싱 의뢰 전 준비할 파일까지 확인하세요.
faq:
  ...
  - q: 보컬에 딜레이를 걸 때 BPM에 맞추는 것이 좋나요?
    a: >-
      ...
      자주 사용합니다. 계산식: 60,000 ÷ BPM = 4분음표 딜레이 ms 값입니다.
  - q: 슬랩백 딜레이란 무엇인가요?
    a: >-
...
딜레이 믹싱에서 중요한 원칙은 딜레이가 원음과 충돌하지 않아야 한다는 것입니다. BPM에 맞지 않는 딜레이, 너무 높은 피드백, 원음보다 밝은 에코 — 이 세 가지가 믹스를 어지럽히는 딜레이의 주요 원인입니다. 반대로 BPM에 정확히 동기화하고, 에코를 원음보다 어둡게(High Cut) 처리하며, Aux 버스로 분리해 조절하면 딜레이는 믹스의 공간과 리듬을 동시에 풍부하게 만들어줍니다.
...
딜레이는 리버브와 함께 믹스에 공간과 깊이를 만드는 두 축입니다. BPM 동기화, 에코 EQ 처리, Aux 버스 분리라는 세 원칙을 지키면 딜레이가 믹스를 어지럽히는 대신 리듬적 생동감과 공간감을 동시에 더해주는 효과적인 도구가 됩니다.

## 딜레이로 곡을 망치는 흔한 순간들

딜레이는 잘 쓰면 곡에 숨을 불어넣지만, 제가 믹스를 받아보면 열에 셋은 딜레이가 오히려 곡을 지저분하게 만들고 있습니다. 가장 흔한 건 "많이 걸수록 넓어진다"는 오해예요. 피드백을 잔뜩 올리고 센드를 크게 열면 공간이 넓어지는 게 아니라, 반복음들이 가사와 겹쳐서 뭉개집니다. 저는 오히려 딜레이를 줄이는 방향으로 손을 대는 경우가 훨씬 많습니다.

두 번째로 자주 보는 건 에코가 원음보다 밝은 경우입니다. 본문에서 강조했듯 에코는 원음보다 어두워야 뒤로 물러나 보이는데, 이 처리를 안 하면 반복음이 원래 목소리보다 앞으로 튀어나와 귀를 찌릅니다. 딜레이 버스에 하이컷 하나만 걸어도 지저분함의 절반은 정리됩니다.

마지막은 딜레이를 곡 전체에 깔아두는 습관입니다. 저는 후렴 끝음이나 감정이 올라오는 한두 마디에만 순간적으로 딜레이를 던지는 방식을 훨씬 선호합니다. 늘 켜져 있는 딜레이는 존재감이 없지만, 조용하다가 딱 한 번 번지는 에코는 듣는 사람 귀에 확실히 남거든요. 결국 딜레이는 양의 문제가 아니라 타이밍의 문제라는 걸, 믹스를 오래 하다 보면 알게 됩니다. 곡을 발매까지 끌고 가면서 이런 공간 처리를 함께 잡고 싶으시면 [발매 프로젝트](/release-project)를 살펴보셔도 좋습니다.
