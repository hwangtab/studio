...
샘플레이트(Sample Rate)는 1초에 오디오 신호를 몇 번 샘플링하는지를 나타내며, 44.1kHz와 48kHz가 가장 일반적으로 사용됩니다. 비트 뎁스는 각 샘플의 진폭 해상도를 결정하며, 24비트로 녹음하면 다이나믹 레인지가 넓어 믹싱 여지가 커집니다. 스트리밍 발매용 최종 파일은 44.1kHz/16비트가 표준이지만, 녹음과 믹싱은 반드시 24비트 이상으로 진행하는 것이 좋습니다.
