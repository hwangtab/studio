...
summary: >-
  음악 라이센싱 정보를 비용, 일정, 준비물 기준으로 비교합니다. Studio NOL 상담 전에 예산과 선택 기준, 의뢰 전 확인할 내용을 빠르게 정리하세요.
faq:
  - q: 음악 라이센싱이란 무엇인가요?
    a: >-
...
유튜브 크리에이터와 소셜미디어 콘텐츠 제작자가 자주 사용하는 플랫폼입니다. 구독자가 연간 정액을 내고 라이브러리 내 모든 음원을 무제한 사용하는 구조입니다. 아티스트에게는 스트림 기반이 아닌 비례 수익 분배(전체 수익의 50% 풀 분배)를 적용합니다. 비독점 계약으로 다른 플랫폼과 동시 등록이 가능합니다.
...
## 싱크 제안이 와도 결국 '쓸 수 있는 마스터'인지에서 갈립니다

라이선싱은 화려해 보이지만, 현장에서 보면 아주 실무적인 문턱이 하나 있습니다. 영상 감독이나 대행사가 곡을 마음에 들어 해도 넘길 수 있는 깨끗한 마스터가 없으면 이야기가 거기서 멈춘다는 거예요. 보컬이 빠진 인스트루멘탈 버전, 길이를 줄인 30초 편집본, 음량 기준을 맞춘 마스터 — 이렇게 '납품 가능한 형태'가 준비된 아티스트가 결국 기회를 잡습니다. 곡을 만들 때부터 인스트루멘탈을 따로 뽑아두는 습관을 권하는 이유입니다.

계약 조건은 제가 함부로 단정할 수 있는 영역이 아니지만, 현장에서 반복해서 보는 실수는 있습니다. 편하다는 이유로 영구·글로벌·독점을 낮은 값에 통째로 넘기는 경우예요. 본문에 정리된 매체·지역·기간·독점성 네 가지는 한 번 내주면 되돌리기 어려운 조건이라, 특히 독점에는 값을 더 받아야 합니다. 이런 문구 해석과 협상은 저 같은 엔지니어가 아니라 음악 전문 변호사의 검토를 꼭 거치시길 권합니다.

곡을 납품 가능한 상태로 다듬는 [발매 프로젝트](/release-project) 단계에서 인스트루멘탈과 마스터를 함께 정리해두면, 이런 제안이 왔을 때 허둥대지 않고 바로 대응할 수 있습니다. 국내에서는 공연권 쪽 등록을 미뤄둔 바람에 마스터 사용료만 받고 저작권료를 놓치는 경우도 있으니, 본문대로 등록은 계약 전에 끝내두시고요. 저는 소리를 넘길 수 있는 상태까지 도와드리고, 그 뒤 계약의 셈법은 전문가에게 맡기시라고 말씀드립니다.
