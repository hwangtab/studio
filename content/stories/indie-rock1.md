...
tags: ["인디 록 레코딩", "어쿠스틱 기타 녹음", "밴드 레코딩", "인디 음악 제작", "어쿠스틱 녹음 기법", "인디 밴드 녹음", "기타 마이킹"]
thumbnail: "/images/service4.webp"
summary: "인디 록·어쿠스틱 기타 레코딩 완전 가이드입니다. 어쿠스틱 기타 마이킹, 인디 록 기타 톤 설계, 보컬-기타 동시 녹음, 밴드 레코딩 배치, 인디 믹싱 전략을 정리합니다."
faq:
  - q: "어쿠스틱 기타 마이킹 위치는 어디가 좋나요?"
    a: "12번 프렛(넥 이음새) 방향 30cm 거리에 마이크를 두는 것이 기본입니다. 사운드홀에 너무 가까이 두면 저역이 과다해집니다. 두 개 마이크로 12번 프렛 + 브릿지 방향을 동시에 잡으면 더욱 풍성한 스테레오 이미지를 얻을 수 있습니다."
  - q: "인디 록 기타 톤은 어떻게 만드나요?"
    a: "소규모 콤보 앰프(Fender Blues Junior, VOX AC15 등)를 가볍게 오버드라이브한 사운드가 인디 록의 전형입니다. 클린 → 크런치 사이의 경계 톤이 핵심이며, 앰프 마이킹과 룸 마이킹을 혼합하면 자연스러운 공간감이 살아납니다."
  - q: "보컬과 기타를 동시에 녹음해도 되나요?"
    a: "데모 녹음 또는 라이브 느낌을 원할 때는 동시 녹음이 가능합니다. 하지만 전문 제작에서는 기타를 먼저 녹음하고 보컬을 후에 레이어하는 오버더빙 방식이 각 트랙의 완성도를 높이는 데 유리합니다."
  - q: "인디 록 믹싱에서 주의할 점은 무엇인가요?"
    a: "과도한 처리를 피하는 것이 인디 록의 매력입니다. 컴프레서는 가볍게, 리버브는 자연스럽게, EQ는 필요한 부분만 조정합니다. 약간의 거칠음(grit)이 장르적 매력이 될 수 있습니다."
---
![인디 록·어쿠스틱 기타 레코딩 완전 가이드 — 스튜디오 놀](/images/service4.webp)

## 인디 록·어쿠스틱 기타 — 날것의 에너지를 담는 레코딩

인디 록과 어쿠스틱 음악은 과도한 프로듀싱보다 진정성 있는 사운드가 핵심입니다. 마이킹과 룸 사운드의 자연스러운 어우러짐이 인디 음악의 매력을 만듭니다.
...
- **문제**: 마이크 블리드 (보컬 마이크에 기타 소리, 기타 마이크에 보컬 소리)

**해결 방법**
1. 물리적 분리: 보컬은 부스, 기타는 라이브 룸에 격리
2. 지향성 마이크 사용: 카디오이드 지향으로 불필요한 방향 차단
3. 마이크 거리 조정: 보컬 마이크는 입에 가까이, 기타 마이크는 앰프에 가까이
4. 라이브 녹음 수용: 약간의 블리드가 인디 록 특유의 라이브 느낌을 줄 수 있음
...
**보컬**
- 컴프레서 가볍게 (1176 패스트 어택)
- 서추레이션으로 따뜻한 배음 추가
- **리버브**: 플레이트 또는 룸 (0.8~1.5초)
...
- 킥에 서추레이션 추가 (딱딱하지 않게)
- 스네어 앰비언스 유지
- 오버헤드 비중 높이기 (사이먼즈·탐보다 심벌 강조)

**전체 믹스**
- 과도한 압축 피하기
- 아날로그 하드웨어 에뮬레이션으로 따뜻함 추가
- 약간의 로파이 질감이 인디 매력
