...
## SSL 버스 컴프는 개별 트랙을 하나로 '접착'하는 글루 컴프예요

채널마다 컴프를 거는 것과 버스 컴프는 목적이 완전히 달라요. 개별 트랙 컴프가 각 악기의 다이나믹을 다듬는 거라면, SSL G-Bus는 이미 정리된 트랙들을 하나의 버스로 묶어 통째로 살짝 눌러서 "따로 놀던 소리들을 한 덩어리로 붙이는" 도구예요. 저는 이걸 접착제, 그래서 '글루(Glue)' 컴프라고 불러요. 마스터 버스에 Ratio 2:1로 아주 얕게 걸어 GR 미터가 -2~3dB만 움직이게 두면, 트랜지언트와 레벨이 은근히 통일되면서 믹스 전체가 처음부터 하나로 녹음된 것처럼 붙어요. 세게 누르는 게 아니라 살짝 안아주는 정도라는 걸 꼭 기억하세요.

여기서 제가 가장 중요하게 보는 건 Attack이에요. 마스터 버스에선 30ms처럼 느긋한 어택을 권하는데, 이유가 있어요. 어택이 1~3ms로 빠르면 킥·스네어의 첫 타격까지 컴프가 붙잡아서 드럼이 뒤로 물러나고 믹스의 생기가 죽거든요. 30ms로 두면 그 초기 타격 파형은 컴프가 반응하기 전에 먼저 빠져나가고, 뒤따르는 서스테인만 가볍게 잡혀 타격감은 살아있으면서 전체 레벨은 안정돼요. Release는 Auto로 두세요. 곡 BPM에 맞춰 리커버리를 알아서 조절해줘서 펌핑 없는 자연스러운 글루가 나와요.

한 가지 순서상의 원칙이 있어요. 드럼 버스에 -6~8dB씩 공격적으로 거는 건 개별 킥·스네어·하이햇이 각자 자리를 잡은 다음에만 하세요. 밸런스가 안 맞은 상태에서 버스 컴프부터 세게 걸면 특정 타악기만 과하게 눌리거나 전체가 지저분하게 뭉쳐요. 그리고 플러그인 고민은 접어두셔도 돼요. Waves SSL G나 Cytomic The Glue면 홈 스튜디오에서 충분히 프로급 결과가 나오고, UAD와의 차이는 최종 음원에서 사실상 구별 못 해요. 마스터 버스 전체를 다루는 큰 그림은 [믹스 버스 가이드](/stories/mix-bus1)에서 이어집니다.
