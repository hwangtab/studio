...
## 홀·플레이트·룸·스프링은 각자 다른 '방'이에요

리버브 종류를 고를 때 스펙표만 보면 다 비슷해 보이는데, 실제로는 완전히 다른 성격의 방이라고 생각하시면 감이 잡혀요. 홀은 천장이 높은 콘서트홀이에요. 디케이가 2초를 넘게 길어서 웅장하지만, 그만큼 소리가 멀어져요. 그래서 발라드나 현악·합창처럼 넓게 퍼져도 되는 소스엔 좋지만, 팝 리드 보컬에 그대로 쓰면 가수가 무대 뒤로 걸어 들어가 버려요.

플레이트는 방이 아니라 금속판이에요. 실제 공간의 반사가 아니라 판을 진동시켜 만든 소리라 뚜렷한 초기 반사음 없이 촘촘하고 매끈한 꼬리만 남아요. 이 특성이 보컬을 앞으로 밀어주면서도 광택을 더해줘서, 팝·K팝 보컬 리버브의 사실상 표준이 됐어요. 룸은 반대로 아주 작은 방이라 디케이가 짧고(1초 안팎) 친밀해서, 드라이한데 살짝만 공기를 넣고 싶을 때 최고예요. 스프링은 기타 앰프에 들어 있는 그 통통 튀는 금속성 울림이라 보컬엔 잘 안 어울리고 서프·레트로 기타에 캐릭터로 써요.

그래서 저는 종류를 먼저 고르지 말고 "이 소스를 앞에 둘까 뒤에 둘까"를 먼저 정하라고 해요. 앞이면 플레이트나 짧은 룸, 뒤로 보내고 싶으면 홀. 방을 상상하면 종류는 저절로 좁혀져요. 리드와 백보컬에 서로 다른 방을 배정하는 원근 감각은 [보컬 믹싱 완전 가이드](/stories/vocal-mixing1)에서 이어집니다.
