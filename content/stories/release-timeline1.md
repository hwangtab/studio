...
summary: >-
  음원 발매 타임라인 완전 가이드입니다. 발매 D-8주부터 D-Day까지 녹음·유통 등록·홍보·발매일 관리 단계별 체크리스트를 정리합니다.
faq:
...
## 타임라인은 발매일에서 거꾸로 세 개의 못을 박는 데서 시작해요

발매 일정을 상담할 때 저는 늘 종이에 발매일 하나만 먼저 찍고, 거기서 거꾸로 세 개의 못을 박자고 해요. 첫 번째 못은 발매 한 달 전, 마스터링 완료. 이게 왜 중요하냐면, 마스터가 늦어지면 뒤의 모든 일정이 도미노처럼 밀리거든요. 아트워크도, 메타데이터 확정도, 유통 등록도 전부 마스터 파일이 나와야 시작되는 일이라, 마스터를 여유 있게 끝내두면 나머지가 안정적으로 돌아가요. 반대로 발매 열흘 전까지 마스터를 붙잡고 있으면, 검토할 시간 없이 그대로 밀어넣게 되고 발매 후에 후회하는 지점이 나와요.

두 번째 못은 발매 7일 전, 스포티파이·애플뮤직 에디토리얼 피칭 마감이에요. 이건 절대 늦으면 안 되는 하드 데드라인이에요. 편집 큐레이터가 곡을 실제로 듣고 검토할 시간이 필요해서, 7일 전이 마지노선이고 이상적으로는 28일 전에 넣는 게 좋아요. 그런데 피칭 권한은 유통사 발매 신청이 승인돼야 열리니까, 역산하면 유통사 제출은 발매 3~4주 전에는 끝나 있어야 해요. 이 순서가 꼬여서 피칭 창을 놓치는 분들이 정말 많아요.

세 번째 못은 발매 직전 48시간, 캠페인 집중 구간이에요. Pre-save 링크로 모아둔 팬들이 발매 순간 동시에 재생되도록 하고, 발매 당일 SNS에 '지금 처음부터 끝까지 한 번 들어주세요'를 구체적으로 요청하는 거예요. 이 초기 완청률과 저장이 알고리즘한테 '이 곡 반응 좋다'는 첫 신호가 되거든요. 이 세 못 사이를 채우는 세부 작업은 위 D-8주 표대로 하나씩 지워나가면 되고, 만에 하나 발매를 미뤄야 하는 상황이 생기면 [음원 재발매·수정·취소·연기 가이드](/stories/re-release1)를 먼저 확인하세요 — 피칭 타이밍이 리셋되는 함정이 있어요.

---

[음원 유통 완전 가이드](/stories/music-distribution1) | [스트리밍 음원 발매 완전 가이드](/stories/streaming-release1) | [플레이리스트 피칭 완전 가이드](/stories/playlist-pitch1) | [음악 마케팅 완전 가이드](/stories/music-marketing1) | [발매 초기 48시간 노출 전략](/stories/streaming-algorithm1) | [Spotify Canvas·가사 완전 가이드](/stories/spotify-canvas1) | [음원 재발매·수정·취소·연기 가이드](/stories/re-release1)
