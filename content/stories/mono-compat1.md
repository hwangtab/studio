...
author: "스튜디오 놀"
category: "음반 제작 가이드"
tags: ["모노 호환성", "모노 체크", "스테레오 믹싱", "블루투스 스피커 믹스", "스마트폰 음질", "페이즈 캔슬레이션", "믹스 모노 확인"]
thumbnail: "/images/portfolio2.webp"
summary: "모노 호환성 믹싱 완전 가이드입니다. 모노 호환성이 중요한 이유, 페이즈 캔슬레이션 발견법, 모노 체크 워크플로우, 스마트폰·블루투스 스피커 최적화 방법을 정리합니다."
faq:
  - q: "모노 호환성이란 무엇인가요?"
    a: "스테레오 믹스를 모노로 합쳤을 때도 원하는 사운드가 유지되는 특성입니다. 스마트폰 스피커, 블루투스 스피커, 라디오, 클럽 시스템은 모노 또는 센터 중심 재생이 많습니다. 모노에서 음량이 줄거나 소리가 사라지면 문제입니다."
  - q: "페이즈 캔슬레이션이란 무엇인가요?"
    a: "L채널과 R채널의 위상이 반대(180도)일 때 모노로 합치면 소리가 상쇄되는 현상입니다. 특히 코러스·플랜저·스테레오 이미저를 과도하게 사용하면 발생합니다. 스테레오에서는 풍부하게 들리지만 모노에서 사라지는 것이 특징입니다."
  - q: "믹스 모노 체크를 어떻게 하나요?"
    a: "DAW 마스터 버스에 유틸리티 플러그인을 삽입하고 모노 버튼을 누르거나, Goniometer/Phase Correlation 미터로 위상을 확인합니다. 모노 체크 시 보컬이 갑자기 작아지거나 베이스가 줄어들면 위상 문제입니다."
  - q: "스마트폰에서 좋은 믹스가 되려면?"
    a: "모노에서도 보컬·킥·스네어가 선명하게 들려야 합니다. 중요한 요소(보컬, 리드, 킥)는 센터에 배치하고, 배경 요소(리버브, 패드, FX)를 사이드에 배치합니다. 스테레오 이미저는 최소한으로 사용하고, 최종 확인은 반드시 스마트폰으로 합니다."
---
