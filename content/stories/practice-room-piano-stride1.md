...
faq:
  ...
  - q: 피아노 스트라이드·뉴올리언스 피아노 연습할 때 무엇을 신경 쓰면 좋나요?
    a: >-
...
## 스트라이드는 왼손 한 손이 밴드 전체를 대신하는 기법이에요

스트라이드가 왜 그렇게 매력적이면서도 어려운지 아세요? 왼손 하나가 베이스와 드럼, 리듬 기타 역할을 동시에 하기 때문이에요. 강박인 1·3박에는 낮은 베이스 음을 '쿵' 짚고, 약박인 2·4박에는 손을 훌쩍 올려 중음역 코드를 '착' 얹는 거죠. 이 낮은 음과 높은 코드 사이를 성큼성큼 걸어다닌다고 해서 stride, 걷는다는 이름이 붙었어요. 피아노 한 대로 파티장을 통째로 채웠던 1920~30년대 할렘의 사운드가 바로 여기서 나옵니다.

이 스타일을 제대로 이해하려면 그 뿌리인 뉴올리언스도 함께 봐야 해요. 스트라이드의 사촌 격인 부기우기는 왼손이 코드를 도약으로 오가는 대신 8분음표로 굴러가는 베이스 라인을 반복하는데, 두 스타일을 나란히 쳐 보면 재즈 피아노의 왼손이 어떻게 발전해 왔는지 손으로 체감이 돼요. Fats Waller의 유머러스한 오른손, James P. Johnson의 클래식한 짜임새, Art Tatum의 초월적인 속도 — 같은 스트라이드라도 사람마다 완전히 다른 언어를 씁니다.

처음 도전하는 분이라면 화려한 10도 도약부터 욕심내지 말고, 낮은 베이스와 코드를 정확히 번갈아 짚는 기본 왕복부터 느리게 다지는 게 순서예요. 도약할 때 코드 손 모양을 공중에서 흐트러뜨리지 않고 그대로 옮겨 가는 감각, 이 첫 단추는 [스트라이드 입문](/stories/practice-room-piano-stride-beginner1)에서 한 박씩 짚어 드립니다.
