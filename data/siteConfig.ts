...
/**
 * 제3자 리스팅은 2026-09-25에 정리했다. 스페이스클라우드(space/68725)는 90일 유입 0·최근
 * 예약 거의 없음·후기 2건이 2025-03이 마지막이었고, 사이트가 같은 방을 직접 예약받기
 * 시작해 더블부킹 위험만 남아 판매를 중지했다(삭제 아님). llms.txt의 후기 문장도 같이 뺐다 —
 * 링크가 죽으면 안 붙인 것보다 나쁘다. 다시 올리면 여기와 llms.ts에 함께 되살릴 것.
 */
...
export const getSiteConfig = (locale: Locale): SiteConfig => {
  return {
    ...
    contact: {
      ...
      //
      // 2026-08-24엔 같은 사업장의 GBP가 둘이었다(옛 CID 5451462716481993990 "스튜디오 놀", 폐업 표시).
      // 2026-09-26 확인: 두 CID가 이제 같은 프로필을 연다(조회수·게시물·소유자 표시 동일, "폐업" 없음).
