...
const homeData = {
    ...
    heroContent: {
      ...
      // 2줄로 줄였다(2026-09-26, 디자인 v2). 4줄이던 때는 390px 모바일에서 부제가 5줄로 감겨
      // 1차 CTA(카카오)가 첫 화면 바닥까지 밀려 있었다. "연신내"는 지역 신호라 남긴다 —
      // seo.title·description에도 있다. \u00A0는 좁은 화면에서 상호가 "스튜디오 / 놀이"로
      // 갈리지 않게.
      // 펀딩 실적·모금액을 히어로에 싣지 않는다(2026-09-26 운영자 판단으로 하루 만에 뺐다). 녹음·연습실로
      // 들어온 방문자에게 "펀딩"과 모금액을 먼저 보여 주면 없던 비용 걱정을 심고, 그 금액이 가격
      // 기준점으로 읽힌다. 펀딩 실적은 펀딩을 찾아 들어온 사람이 보는 /release-project·/crowdfunding-design에만 둔다.
      subtitle: "기획·녹음·믹싱부터 유통·국내외 홍보까지,\n그 다음은 연신내 스튜디오\u00A0놀이 함께 만듭니다.",
