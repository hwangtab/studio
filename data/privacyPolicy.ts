    ...
    sections: [
      ...
      },
      {
        ...
        body:
          ...
          '상태에서는 정산 정보 저장 자체를 거부하고, 평문으로 대신 받아 두지 않습니다. 은행명과 ' +
          '예금주, 계좌번호 전체는 운영자가 이체하려고 조회 버튼을 눌렀을 때만 복호화되어 그 응답으로 ' +
          '나오며, 그 조회는 성공·실패를 가리지 않고 접속기록에 남습니다(아래 19항). 심사 화면과 ' +
          '프로젝트 목록에는 암호문조차 싣지 않습니다. 개설자 본인의 편집 화면에는 계좌가 등록되었다는 ' +
          '사실과 계좌번호 뒤 4자리만 나오고, 정산 안내 메일에도 계좌번호는 뒤 4자리까지만 나갑니다. ' +
      ...
      // 그중 하나(개설자 배송 목록)는 수행자가 관리자가 아니라 개설자다.
      {
        ...
        items: [
          ...
          '접근 권한의 제한 — 개인정보를 볼 수 있는 관리자 화면은 비밀번호 인증을 통과한 세션에서만 열립니다. 주민등록번호와 정산 계좌(은행명·계좌번호·예금주)는 운영자가 그 값을 보려고 조회를 요청한 응답에만 실리며, 목록·심사 화면과 페이지 소스, 안내 메일, 내려받는 파일에는 담기지 않습니다',
          '접속기록의 보관 — 주민등록번호 조회, 정산 계좌 조회, 정산을 기록하기 직전에 주민등록번호와 정산 계좌가 각각 실제로 열리는지 확인하는 두 가지 점검, 그리고 개인정보가 담긴 파일을 내려받는 일은 성공·실패를 가리지 않고 수행자·행위 종류·대상·결과·IP 주소·시각으로 기록해 2년 동안 보관합니다. 열람하거나 내보낸 값 자체는 기록하지 않습니다. 관리자 인증이 단일 비밀번호 하나여서 관리자 쪽 수행자는 개인이 아니라 "관리자"로 기록되며, 여러 사람이 같은 비밀번호를 쓰면 이 기록으로는 누구인지 가릴 수 없습니다',
