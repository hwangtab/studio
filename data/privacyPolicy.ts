...
export const POLICY_COPY_BY_LOCALE: Record<Locale, PolicyCopy> = {
  ko: {
    ...
    sections: [
      ...
      // db/schema.ts와 두 크론(pages/api/cron/purge-funding.ts 매월 2일,
      // pages/api/cron/purge-orders.ts 매월 3일)에서 확인한 것이다 — 코드가 하지 않는 파기를
      // 적지 않는다. **보존 항목 열거는 파기가 늘어날 때마다 줄어든다**: 자동 파기 ④~⑩이
      ...
      // 이 문서가 실제보다 많이 들고 있다고 고지하는 셈이 된다.
      {
        ...
        body:
          ...
          '작업이 대상에 따라 두 벌로 나뉘어, 후원·개설자·접속기록 쪽은 매월 2일에, 주문·예약· ' +
          '믹싱·정기결제와 결제 처리 기록 쪽은 매월 3일에 각각 돕니다. 방법은 데이터베이스에 ' +
        ...
        items: [
          ...
          '자동 파기 ⑦ 구독 해지 사유 — 해지한 날부터 3년이 지나면 지웁니다. 해지 사유에는 서비스에 대한 불만이 적힐 수 있어, 소비자의 불만·분쟁 처리 기록에 정해진 기간과 같은 3년으로 둡니다',
          '자동 파기 ⑧ 예약·믹싱 신청의 요청사항 — 자유롭게 적는 칸이라 계약 내용을 특정하는 항목이 아니므로 5년이 아니라 3년입니다. 기산점은 예약 이용일(취소했으면 취소일)이고, 믹싱은 납품일(취소했으면 취소일)입니다',
          '자동 파기 ⑨ 결제사가 보낸 응답 원문 — 결제 승인 응답 원본은 승인일부터 5년이 지나면 지웁니다. 대사(對査)에 필요한 결제 식별값·결제수단·승인 시각·영수증 주소는 별도 항목으로 남습니다. 결제가 거절된 사유의 원문은 주문 결제와 정기결제 회차 모두 1년이 지나면 지우며, 결제사가 정한 사유 코드는 남깁니다. 정기결제 카드 등록 응답 원본은 그 카드를 더 쓸 수 없게 된 때(등록 해지 또는 구독 종료)에 지웁니다 — 카드사 이름과 마스킹된 카드번호, 등록 시각은 과거 회차의 결제 근거로 남습니다',
