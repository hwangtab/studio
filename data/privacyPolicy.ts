...
export const POLICY_COPY_BY_LOCALE: Record<Locale, PolicyCopy> = {
  ko: {
    ...
    sections: [
      ...
      // taxType === 'withholding'일 때만 저장하고 'invoice'면 기존 값까지 지우는 그대로이고,
      ...
      // 자동 파기 절차는 lib/funding/retention.ts가 이 컬럼을 건드리지 않으므로 없다.
      {
        ...
        items: [
          ...
          '파기 — 개설자가 편집 화면에서 세금 처리 구분을 사업자로 바꿔 저장하면 등록된 번호가 함께 지워집니다. 원천징수 대상이 아니게 되는 순간 보관할 근거가 사라지기 때문입니다. 그 밖의 삭제 요청은 위 5항에 따라 처리합니다',
