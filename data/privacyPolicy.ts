    ...
    sections: [
      ...
      },
      {
        ...
        body:
          ...
          '원천징수세액의 신고·납부에 씁니다. 은행명·계좌번호·예금주는 데이터베이스에 넣기 전에 ' +
          '한 벌로 묶어 암호화(AES-256-GCM)하며, 평문으로 두는 것은 계좌번호 뒤 4자리뿐입니다 — ' +
          '계좌가 등록되었는지와 그 뒤 4자리를 복호화 없이 확인하기 위한 것입니다. 암호화 키가 없는 ' +
          '상태에서는 정산 정보 저장 자체를 거부하고, 평문으로 대신 받아 두지 않습니다. 복호화되는 ' +
          '경로는 셋입니다. 하나는 운영자가 이체하려고 관리자 화면에서 조회 버튼을 누른 때로, 이때 은행명과 ' +
          '예금주, 계좌번호 전체가 그 응답에만 실립니다. 다른 하나는 정산을 기록하기 직전에 등록된 계좌가 ' +
          '실제로 열리는지 확인하는 점검으로, 성공 여부만 보고 값을 화면·메일·응답 어디에도 내보내지 ' +
          '않습니다. 나머지 하나는 정산 안내 메일을 만드는 때입니다 — 이 메일 본문에는 은행명과 예금주가 ' +
          '그대로 실리고, 계좌번호는 뒤 4자리까지만 나갑니다. 어느 계좌로 보냈는지 개설자가 확인할 수 ' +
          '있어야 하기 때문입니다. 세 경로 모두 성공·실패를 가리지 않고 접속기록에 남습니다(아래 19항). ' +
          '심사 화면과 프로젝트 목록에는 암호문조차 싣지 않습니다. 개설자 본인의 편집 화면에는 계좌가 ' +
          '등록되었다는 사실과 계좌번호 뒤 4자리만 나옵니다. ' +
          ...
          '마찬가지여서, 정산이 끝나도 계좌 정보는 암호화된 채로 계정에 남아 있습니다. 개설자는 편집 화면에서 ' +
      ...
      // 이 문서가 실제보다 많이 들고 있다고 고지하는 셈이 된다.
      {
        ...
        body:
          ...
          '지워집니다.',
        items: [
          '자동 파기 ① 후원자의 배송·메모 정보 — 리워드 전달이 끝나고 1년이 지났으며 아래 법정 보존 기간도 지난 후원 건에서, 받는 분 이름·연락처·우편번호·주소·상세주소·배송 메모와 응원 메시지, 명단에 표시할 이름(가린 이름·닉네임), 운영자가 적은 메모를 지웁니다. 명단에 표시할 이름을 지운 후원은 서포터 명단에서도 내려갑니다. 후원 기록의 행 자체는 남습니다 — 아래 보존 대상 항목이 그 행에 있기 때문입니다',
          '자동 파기 ② 개설자 주민등록번호 — 위 15항이 정한 요건을 갖추면 암호문을 지웁니다',
          '자동 파기 ③ 고유식별정보 접속기록 — 기록한 때로부터 2년이 지난 행을 지웁니다(아래 19항)',
          '자동 파기 ④ 주문자의 이름·연락처·이메일 주소 — 문의가 아니라 주문(예약·믹싱·펀딩·정기결제)으로 받은 세 항목은 아래 법정 보존 기간인 5년이 지나면 표시로 덮어씁니다. 기산점은 주문한 날과 그 주문이 마지막으로 바뀐 날(환불·취소 포함) 가운데 나중 쪽이라, 결제한 지 한참 뒤에 환불된 건도 그 사건을 기준으로 셉니다. 펀딩 주문이면 그때 그 후원의 서포터 명단 표시 이름(가린 이름·닉네임)과 응원 메시지도 함께 지웁니다 — 리워드 전달이 기록되지 않아 위 ①이 시작되지 않은 후원도 여기서 끝납니다. 주문 기록의 행 자체는 남습니다',
      ...
      // 암호화는 lib/crypto/fieldCrypto.ts(AES-256-GCM, 주민등록번호 컬럼과 정산 계좌 봉투
      // lib/funding/payoutAccountCrypto.ts), 관리자 인증은
      // lib/contracts/admin-auth.ts(사람별 비밀번호 + 세션, lib/contracts/admin-accounts.ts),
      // 접속기록은 lib/privacy/accessLog.ts,
    ...
    sections: [
      ...
      },
      {
        ...
        body:
          ...
          'no end date of its own, so we treat it as ended once three years have passed with no ' +
          'activity of any kind - no charge attempted, no card registered or changed, nothing else ' +
          'recorded against it. If no payment was ever taken, no contract was formed and no money ' +
          'changed hands, so there is no transaction record to keep: we erase the name, phone number, ' +
          'email address and any supporter display name at the same time. If payments were taken, we ' +
          'only mark the subscription as ended, and the five-year rule above then runs from that last ' +
    ...
    sections: [
      ...
      },
      {
        ...
        body:
          ...
          '若满三年没有任何活动——未尝试扣款，未登记或更换银行卡，也没有其他记录变动——我们即视为已结束。' +
    ...
    sections: [
      ...
      },
      {
        ...
        body:
          ...
          'damos por terminado cuando han pasado tres años ' +
    ...
    sections: [
      ...
      },
      {
        ...
        body:
          ...
          'tự nó không có ngày kết thúc; vì vậy nếu suốt ba năm không có bất kỳ hoạt động nào - ' +
    ...
    sections: [
      ...
      },
      {
        ...
        body:
          ...
          'ย่อมไม่มีวันสิ้นสุดของตัวเอง ดังนั้นหากครบสามปีโดยไม่มีความเคลื่อนไหวใด ๆ คือไม่มีการพยายามเรียกเก็บเงิน ' +
    ...
    sections: [
      ...
      },
      {
        ...
        body:
          ...
          'to\'lovning o\'z tugash sanasi bo\'lmaydi. Shuning uchun uch yil davomida hech qanday ' +
