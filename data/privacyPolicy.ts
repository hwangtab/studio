    ...
    sections: [
      ...
      // 이 문서가 실제보다 많이 들고 있다고 고지하는 셈이 된다.
      {
        ...
        items: [
          ...
          '자동 파기 ④ 주문자의 이름·연락처·이메일 주소 — 문의가 아니라 주문(예약·믹싱·펀딩·정기결제)으로 받은 세 항목은 아래 법정 보존 기간인 5년이 지나면 표시로 덮어씁니다. 기산점은 주문한 날과 그 주문이 마지막으로 바뀐 날(환불·취소 포함) 가운데 나중 쪽이라, 결제한 지 한참 뒤에 환불된 건도 그 사건을 기준으로 셉니다. 펀딩 주문이면 그때 그 후원의 받는 분 이름·연락처·우편번호·주소·상세주소·배송 메모, 운영자가 적은 메모, 응원 메시지, 서포터 명단 표시 이름(가린 이름·닉네임)도 함께 지웁니다 — 리워드 전달이 기록되지 않아 위 ①이 시작되지 않은 후원도 여기서 끝납니다. 주문 기록의 행 자체는 남습니다',
