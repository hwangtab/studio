...
export const SERVICE_DATA_PROCESSORS: ReadonlyArray<{ name: string; purpose: string; items: string }> = [
  { name: 'Resend', purpose: '문의 접수 알림 메일(스튜디오 앞) 발송, 예약·주문 확정·취소 안내 메일 발송, hello@ 수신 메일의 운영자 전달', items: '이름, 연락처, 이메일 주소, 문의 내용, 예약·주문 내역, hello@ 수신 메일 본문·첨부파일' },
  ...
  ko: {
    ...
    sections: [
      ...
      },
      {
        ...
        body: '개인정보를 제3자에게 제공하지 않습니다. 다만 문의·상담과 예약·결제 처리에 필요한 범위에서 아래와 같이 개인정보 처리를 위탁하고 있으며, 수탁자가 바뀌면 이 처리방침으로 알립니다. 펀딩(리워드 선주문) 처리의 위탁 현황은 아래 9항이, 언론 홍보 업무의 매체 연락처는 아래 10~12항이, 개설자 계정 정보는 아래 13~15항이, 요청 제한 기록은 아래 16항이 따로 정합니다.',
      ...
      },
      {
        ...
        body:
          ...
          '정산 정보를 다시 입력해 언제든 덮어쓸 수 있습니다. 삭제를 요청하시면 위 5항에 ' +
      ...
      // 열람 경로는 pages/api/admin/funding/projects/[id]/resident-number.ts 하나뿐이며,
      // 자동 파기 절차는 lib/funding/retention.ts가 이 컬럼을 건드리지 않으므로 없다.
      {
        ...
        body:
          ...
          '지급명세서를 제출해야 하는데, 지급명세서는 소득을 받는 분을 주민등록번호로 적도록 되어 ' +
          '있기 때문입니다. 근거는 소득세법과 그 시행령이 정한 원천징수의무자의 원천징수세액 신고·납부 ' +
          '의무와 지급명세서 제출 의무이며, 이 목적 외에는 어떤 경우에도 이용하지 않습니다. ' +
        ...
        items: [
          ...
          '근거 법령 — 소득세법과 그 시행령이 정한 원천징수의무자의 신고·납부 의무와 지급명세서 제출 의무. 개인정보 보호법은 주민등록번호를 법령에 구체적인 근거가 있는 경우에만 처리하도록 정하고 있으며, 스튜디오 놀은 이 의무를 그 근거로 삼아 원천징수 대상 개설자에 한해서만 수집합니다',
          '안전조치 — 데이터베이스에 넣기 전에 암호화(AES-256)해 저장하며, 평문 상태로는 저장하지 않습니다. 개인정보 보호법이 정한 주민등록번호 암호화 보관 의무에 따른 조치입니다',
          '열람 범위 — 스튜디오 운영자가 신고·지급명세서 제출을 위해 관리자 화면에서 조회 버튼을 누른 그 순간에만 복호화해 봅니다. 조회한 사실은 서버 기록에 남기되 번호 자체는 기록하지 않습니다. 개설자 본인의 편집 화면, 프로젝트 목록·심사 화면, 안내 메일, 내려받는 파일 어디에도 번호는 나오지 않으며 등록 여부(등록됨·미등록)만 표시됩니다',
          '보유 기간 — 계정에 등록되어 있는 동안 보관하며, 기간이 지나면 자동으로 지우는 절차는 두고 있지 않습니다. 정산이 끝나도 남아 있습니다',
          '파기 — 개설자가 편집 화면에서 세금 처리 구분을 사업자로 바꿔 저장하면 등록된 번호가 함께 지워집니다. 원천징수 대상이 아니게 되는 순간 보관할 근거가 사라지기 때문입니다. 다만 그 개설자에게 이미 원천징수한 정산이 있으면 지우지 않고 보관합니다 — 이미 떼어 간 세액을 신고하고 그에 대한 지급명세서를 제출할 의무가 남아 있어, 그 의무가 보관 근거가 되기 때문입니다. 그 밖의 삭제 요청은 위 5항에 따라 처리합니다',
