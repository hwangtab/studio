    ...
    sections: [
      ...
      // 내려받기와 화면 열람이 같은 action을 쓴다 — 나가는 항목도 건수도 같기 때문이다.
      {
        ...
        items: [
          ...
          '접속기록의 보관 — 주민등록번호 조회, 정산 계좌 조회, 정산을 기록하기 직전에 주민등록번호와 정산 계좌가 각각 실제로 열리는지 확인하는 두 가지 점검, 정산 안내 메일을 만들며 계좌를 여는 일, 그리고 개인정보가 담긴 목록을 파일로 내려받거나 화면으로 여는 일은 성공·실패를 가리지 않고 수행자·행위 종류·대상·결과·IP 주소·시각으로 기록해 2년 동안 보관합니다. 열람하거나 내보낸 값 자체는 기록하지 않습니다. 관리자 쪽 수행자는 사람마다 다른 비밀번호로 갈려 있어 개인별 식별자로 기록됩니다. 다만 비밀번호를 사람별로 나누지 않고 하나로 운영하는 동안에는 개인이 아니라 "관리자"로만 기록되며, 그때는 여러 사람이 같은 비밀번호를 쓰면 이 기록으로 누구인지 가릴 수 없습니다',
