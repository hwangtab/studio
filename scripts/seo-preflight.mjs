...
const traps = [
  ['앵커(#) 행', 'page-all/page-query의 "…slug#섹션" 행은 별개 페이지가 아니라 목차 점프링크다. '
    + '노출의 34%가 앵커다(2026-09-23 실측, 28일). 제외하지 않으면 노출이 부풀고 CTR이 낮게 나온다. '
    + '⚠ base 페이지에 합치지 말고 버릴 것 — split("#")[0] 정규화는 같은 노출을 두 번 세어 2~4배로 뛴다.'],
  ['--from-log 기본 대조군', 'ctr-verdict --from-log가 붙이는 producer1·daw-choice1·practice-room-monthly1은 '
    + '장부가 2026-09-11에 부적격으로 지정한 셋이다(practice-room-monthly1은 9/15부터 자기도 실험 중). '
    + '그대로 읽으면 판정이 뒤집힌다 — 2026-09-24 highnote1이 실제로 그랬다(부적격 대조군 기준 20%p 열위 → '
    + '계열 대조군 8편 기준 중립). 처치군과 같은 주제 계열에서 대조군을 직접 지정할 것.'],
