...
const prosePool = [
  ...
  `{{REGION}} 아티스트들이 스튜디오 놀을 방문할 때 주로 활용하는 대중교통 루트는 지역 내 주요 역이나 정류장에서 서울 방향 노선을 이용하는 방식입니다. 연신내역은 6호선과 3호선이 교차하는 환승역이어서 서울 각지에서 접근성이 뛰어납니다. {{REGION}}에서 출발하는 경우 혼잡 시간대를 피해 오전 중에 이동하면 이동 피로를 최소화할 수 있습니다. 스튜디오 놀은 연신내역 4번 출구에서 걸어서 5분 거리에 있어 짐이 많은 뮤지션도 부담 없이 찾아올 수 있습니다.`,
  ...
  `{{REGION}}에서 연신내 스튜디오 놀까지 이동 경로를 처음 이용하는 뮤지션이라면 출발 전 네이버 지도에서 '연신내역'을 검색해 경로를 미리 저장해두는 것을 권장합니다. 이동 중 환승역을 혼동하지 않도록 경로를 캡처해두거나 오프라인 지도를 활용하면 편리합니다. {{REGION}} 지역 특성상 서울 접근 경로가 다양할 수 있으므로 본인의 출발지에서 가장 빠른 경로를 선택하는 것이 중요합니다. 연신내역에 도착하면 4번 출구로 나와 도보 5분이면 스튜디오에 도착합니다.`,
