...
import { fixedSubscriptionAmounts, subscriptionOrderName } from './amounts';

describe('subscriptionAmounts', () => {
  it('연습실은 상수 + VAT 10%', () => {
    expect(fixedSubscriptionAmounts('practice-room')).toEqual({
  ...
  it('레슨도 같은 규칙 — 표기액 + VAT 10%', () => {
    expect(fixedSubscriptionAmounts('lesson')).toEqual({
  ...
  it('연습실과 레슨은 같은 VAT 규칙을 쓴다', () => {
    const room = fixedSubscriptionAmounts('practice-room');
    const lesson = fixedSubscriptionAmounts('lesson');
  ...
  it('상수가 바뀌면 청구액도 따라온다 — 리터럴을 박아 두지 않았다는 확인', () => {
    expect(fixedSubscriptionAmounts('practice-room').itemAmount).toBe(PRACTICE_ROOM_MONTHLY_PRICE);
  });
