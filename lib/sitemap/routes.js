...
// /guides/[slug]는 동적 라우트라 pageRouteMap 방식이 안 맞는다 — 허브 콘텐츠의
// 단일 소스인 data/buyerIntentHubs.ts mtime을 lastmod로 쓴다(허브 카피 수정 시에만 갱신).
const buyerIntentHubsDataFile = path.join(process.cwd(), 'data', 'buyerIntentHubs.ts');
...
const getRouteLastmod = (routePath, buildTimestamp) => {
  ...
  if (pathWithoutLocale.startsWith('/guides/')) {
    return toIsoMtime(buyerIntentHubsDataFile) || buildTimestamp;
  }
