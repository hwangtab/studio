...
const { readFundingProjects } = require('./fundingMeta');
const sitemapConfig = require('../../next-sitemap.config.js');

describe('readFundingProjects — draft·hidden frontmatter 파싱', () => {
  it('smoke-test 프로젝트는 항상 hidden: true로 읽힌다 (draft 여부는 검증 시 토글된다)', () => {
    const smokeTest = readFundingProjects().find((p) => p.slug === 'smoke-test');
    expect(smokeTest).toMatchObject({ hidden: true });
    expect(typeof smokeTest.draft).toBe('boolean');
    expect(smokeTest.lastmod).toMatch(/^\d{4}-\d{2}-\d{2}T00:00:00\+09:00$/);
  });
});
...
describe('exclude — 런타임 사이트맵 라우트는 페이지로 등재하지 않는다', () => {
  it('/sitemap-funding.xml은 exclude에 있다 (2026-09-21 프로덕션에서 <url>로 실려 있었다)', () => {
    expect(sitemapConfig.exclude).toContain('/sitemap-funding.xml');
  });
});
