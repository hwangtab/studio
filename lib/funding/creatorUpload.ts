...
export const UPLOAD_LIMITS = {
   ...
   * 프로젝트당 누적 장수.
   *
   * **저장된 것만 센다** — 본문·표지·리워드 이미지에서 이 주소를 찾아 세는 방식이다
   * (`countProjectMedia`). 올렸다가 본문에서 지운 이미지는 Blob에 남아 이 수에 안 들어가지만,
   * 월 1회 크론이 어느 프로젝트도 참조하지 않는 파일을 지운다(`lib/funding/mediaRetention.ts`,
   * 업로드 7일 유예). 지워지기 전이라도 주소를 아는 것만으로는 열리지 않는다 — 미디어
   * 라우트가 승인된 참조·소유 개설자·관리자만 통과시킨다(`lib/funding/mediaAccess.ts`).
   *
   * 교체·반려 시점의 동기 삭제는 두지 않았다. 청소가 다 덮고, 반려된 프로젝트의 행은
   * 기록이라 그 이미지도 참조로 본다.
   */
  maxPerProject: 30,
} as const;
