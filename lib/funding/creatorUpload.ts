...
export const UPLOAD_LIMITS = {
   ...
   * (`countProjectMedia`). 올렸다가 본문에서 지운 이미지는 Blob에 남지만 이 수에 안 들어가고,
   * **지우는 경로도 없다.** 그래서 고아 파일은 저장소에 계속 쌓인다(주소를 아는 사람은
   * 계속 볼 수 있다 — 반려된 프로젝트의 이미지도 마찬가지다). 삭제 경로를 만들려면
   * "누가 언제 무엇을 지울 수 있는가"를 먼저 정해야 해서 여기서는 열지 않았다.
   */
  maxPerProject: 30,
} as const;
