 ...
 * 비콘(`/api/payments/failed`, Origin 검사 + IP 분당 20회) 하나다 — 펀딩·예약 실패 화면은
 * 2026-09-25에 비콘으로 옮겼다(무인증 GET 하나로 남의 주문 기록이 덮이던 경로였다).
 * Origin 검사는 브라우저가 붙이는 헤더에 기대는 것이라 형태 검증을 대신하지 못한다.
 * 느슨하면 남의 주문번호를 넣어 보며 상태를 떠보거나, 임의 문자열을 우리 DB에 심을 수 있다.
 */
