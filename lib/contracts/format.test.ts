...
/**
 * 오전·오후 표기가 실행 환경의 로케일 데이터에 좌우되지 않아야 한다.
 *
 * 프로덕션에서 관리자 화면 한 장에 "오후 03:19"(브라우저)와 "PM 03:19"(서버)가 함께
 * 찍힌 적이 있다. Vercel 런타임 ICU에 한국어 dayPeriod가 없어서인데, 계약서 PDF의 서명
 * 일시도 서버에서 만들어지므로 같은 값이 인쇄되고 있었다. 서명 시각은 이 문서가 증명하려는
 * 것의 핵심이라 렌더 위치에 따라 달라지면 안 된다.
 */
describe('시각 표기가 환경에 좌우되지 않는다', () => {
  const at = (iso: string) => formatDateTime(new Date(iso));

  it('영어 오전/오후 표기가 섞이지 않는다', () => {
    for (const hour of [0, 6, 11, 12, 13, 18, 23]) {
      const text = at(`2026-09-01T${String(hour).padStart(2, '0')}:30:00.000Z`);
      expect(text).not.toMatch(/\b[AP]M\b/);
      expect(text).toMatch(/오전|오후/);
    }
  });

  it.each([
    ['KST 자정은 오전 12시', '2026-08-31T15:00:00.000Z', '오전 12:00'],
    ['KST 정오는 오후 12시', '2026-09-01T03:00:00.000Z', '오후 12:00'],
    ['KST 오전 1시', '2026-08-31T16:05:00.000Z', '오전 01:05'],
    ['KST 오후 11시', '2026-09-01T14:45:00.000Z', '오후 11:45'],
  ])('%s', (_label, iso, expected) => {
    expect(at(iso)).toContain(expected);
  });
});
