...
/**
 * 계약서가 법에서 요구하는 조항을 빠뜨리지 않았는지 본다.
 *
 * 2026-09-01 감사에서 네 가지가 빠져 있었다. 셋은 방문판매법 제30조 제1항이 계속거래
 * 계약서에 적으라고 정한 항목(사업자 이메일·해지권 행사 서식·소비자피해 처리 절차)이고,
 * 하나는 전자문서법 제7조 제2항 제1호가 말하는 "미리 합의한 절차"다. 월 이용료가
 * 시행령 제37조의 기준(10만원)을 넘으므로 이 계약은 계속거래에 해당한다.
 *
 * 더 무거운 것은 제32조 제1항이었다 — "실제 공급된 대가를 초과하여 수령한 대금의 환급을
 * 부당하게 거부하여서는 아니 된다"는 강행규정인데, 계약서 세 곳이 미이용분을 돌려주지
 * 않겠다고 적고 있었다. 서명 절차를 아무리 잘 입증해도 강행규정 앞에서는 조항이 무효라,
 * 여기가 무너지면 전자계약 시스템 전체가 헛일이 된다. 문구를 손볼 때 이 검사를 지우지 말 것.
 */
describe('법에서 요구하는 조항', () => {
  const content = buildContractContent(baseData);

  it.each([
    ['사업자 이메일 (방판법 §30①1호)', 'hello@studionol.co.kr'],
    ['해지권 행사 서식 (방판법 §30①6호)', '계약 해지 통지서'],
    ['소비자피해 처리 (방판법 §30①7호)', '1372 소비자상담센터'],
    ['해지권 (방판법 §31)', '계약 기간 중 언제든지 계약을 해지'],
    ['전자서명 효력 약정 (전자서명법 §3②)', '서명·기명날인과 동일한 효력'],
    ['본인확인 절차 합의 (전자문서법 §7②1호)', '을 본인이 한 것으로 본다'],
  ])('%s 조항이 있다', (_label, needle) => {
    expect(content).toContain(needle);
  });

  it('미이용 기간분을 일할 계산해 반환한다고 적는다 (방판법 §32①)', () => {
    expect(content).toContain('일할 계산');
    expect(content).toContain('이용하지 않은 일수에 해당하는 금액');
  });

  /** 환급을 거부하는 문구가 되살아나면 제32조 제1항 위반으로 되돌아간다. */
  it('환급을 무조건 거부하는 문구가 없다', () => {
    expect(content).not.toContain('이미 납부한 이용료는 반환하지 않는다');
    expect(content).not.toContain('해당 월의 이용료는 반환하지 아니한다');
